...
begin
    if MessageBox(Self.Handle, 'Confirma a eliminação da nota fiscal?', 'Confirmação', MB_YESNO + MB_ICONQUESTION) = 6 then
    begin
...
begin
  if MessageBox(Self.Handle, 'Deseja apagar a pré-carga?', 'Confirmação', MB_YESNO + MB_ICONQUESTION) = 6 then
   begin
        with qrAux do begin
             Close;
             sql.Clear;
