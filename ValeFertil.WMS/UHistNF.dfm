object FHistNF: TFHistNF
  Left = 193
  Top = 73
  Width = 809
  Height = 466
  BorderIcons = []
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label7: TLabel
    Left = 49
    Top = 25
    Width = 80
    Height = 15
    Caption = 'Nº Nota Fiscal '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label8: TLabel
    Left = 129
    Top = 25
    Width = 80
    Height = 15
    Caption = 'Nº Nota Fiscal '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsItalic]
    ParentFont = False
  end
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 793
    Height = 48
    Align = alTop
    BevelInner = bvLowered
    Color = clWhite
    TabOrder = 0
    object SpeedButton3: TSpeedButton
      Left = 752
      Top = 3
      Width = 37
      Height = 43
      Hint = 'Sair'
      Anchors = [akTop, akRight]
      Caption = 'Sair'
      Flat = True
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      Spacing = 0
      OnClick = SpeedButton3Click
    end
    object Label16: TLabel
      Left = 9
      Top = 9
      Width = 301
      Height = 29
      Caption = 'Histórico de Nota Fiscal'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -24
      Font.Name = 'Georgia'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object SBT_VISU: TSpeedButton
      Left = 696
      Top = 3
      Width = 55
      Height = 43
      Hint = 'Visualizar Relatório'
      Anchors = [akTop, akRight]
      Caption = 'Relatório'
      Flat = True
      Glyph.Data = {
        
        6464686E6E828282828282828282}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      Spacing = 0
      OnClick = SBT_VISUClick
    end
    object SpeedButton2: TSpeedButton
      Left = 649
      Top = 3
      Width = 49
      Height = 43
      Hint = 'Exportar Excel'
      Caption = 'Excel'
      Flat = True
      Glyph.Data = {
        
        171717171717521B7D1D1D1D66660F0F0F191941530117171717}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton2Click
    end
  end
  object GRID1: TwwDBGrid
    Left = 0
    Top = 117
    Width = 793
    Height = 282
    Selected.Strings = (
      'NFI_NUMERO'#9'9'#9'Nota Fiscal'
      'OS_ID'#9'6'#9'OS'
      'MANI_ID'#9'10'#9'Manifesto'
      'TIPO'#9'15'#9'Tipo'
      'NFI_DEMI'#9'9'#9'Dt.Entrada'
      'NFI_DTSAIDA'#9'9'#9'Dt. Entrega'
      'NFI_ENTREGA'#9'20'#9'Situação'
      'MOT_NOME'#9'28'#9'Motorista'
      'VEIC_NOME'#9'18'#9'Tp. Veículo'
      'REG_NOME'#9'40'#9'Região'#9'F'
      'NFI_QTDEUV'#9'13'#9'Qtde. UV'
      'NFI_PBRU'#9'12'#9'Peso'
      'NFI_TOTA'#9'12'#9'Vlr. da Nota')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    Align = alClient
    Color = clWhite
    DataSource = DS_HIST
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Arial Narrow'
    Font.Style = []
    KeyOptions = []
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentFont = False
    TabOrder = 1
    TitleAlignment = taCenter
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -11
    TitleFont.Name = 'Arial'
    TitleFont.Style = [fsBold]
    TitleLines = 1
    TitleButtons = False
    OnCalcCellColors = GRID1CalcCellColors
  end
  object Panel7: TPanel
    Left = 0
    Top = 48
    Width = 793
    Height = 69
    Align = alTop
    BevelInner = bvLowered
    BevelOuter = bvSpace
    TabOrder = 2
    object Label4: TLabel
      Left = 48
      Top = 6
      Width = 44
      Height = 16
      Caption = 'Filtros:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object mmFiltros: TMemo
      Left = 108
      Top = 8
      Width = 597
      Height = 53
      BorderStyle = bsNone
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
  end
  object Panel8: TPanel
    Left = 0
    Top = 399
    Width = 793
    Height = 29
    Align = alBottom
    TabOrder = 3
    object Label1: TLabel
      Left = 107
      Top = 10
      Width = 44
      Height = 15
      Caption = 'Entrada'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label3: TLabel
      Left = 245
      Top = 10
      Width = 32
      Height = 15
      Caption = 'Saída'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label48: TLabel
      Left = 375
      Top = 10
      Width = 59
      Height = 15
      Caption = 'Devolução'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label2: TLabel
      Left = 525
      Top = 10
      Width = 59
      Height = 15
      Caption = 'Reentrega'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label5: TLabel
      Left = 684
      Top = 10
      Width = 51
      Height = 15
      Caption = 'Entregue'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Panel2: TPanel
      Left = 36
      Top = 7
      Width = 68
      Height = 18
      BevelInner = bvLowered
      BevelOuter = bvSpace
      Color = 16777128
      TabOrder = 0
      Visible = False
    end
    object Panel5: TPanel
      Left = 174
      Top = 7
      Width = 68
      Height = 18
      BevelInner = bvLowered
      BevelOuter = bvSpace
      Color = clTeal
      TabOrder = 1
      Visible = False
    end
    object Panel1: TPanel
      Left = 304
      Top = 7
      Width = 68
      Height = 18
      BevelInner = bvLowered
      BevelOuter = bvSpace
      Color = clRed
      TabOrder = 2
      Visible = False
    end
    object Panel4: TPanel
      Left = 454
      Top = 7
      Width = 68
      Height = 18
      BevelInner = bvLowered
      BevelOuter = bvSpace
      Color = clOlive
      TabOrder = 3
      Visible = False
    end
    object Panel6: TPanel
      Left = 613
      Top = 7
      Width = 68
      Height = 18
      BevelInner = bvLowered
      BevelOuter = bvSpace
      Color = clLime
      TabOrder = 4
      Visible = False
    end
  end
  object Q_HIST: TwwQuery
    AutoCalcFields = False
    DatabaseName = 'NBFDados'
    SessionName = 'Sec_BaseDados'
    SQL.Strings = (
      
        'Select A.*, C.REG_NOME from HISTORICO_NF A LEFT OUTER JOIN MANIF' +
        'ESTO B ON A.MANI_ID = B.MANI_ID'
      'LEFT OUTER JOIN REGIAO C ON B.REG_ID = C.REG_ID'
      'order by NFI_CODI, NFI_DEMI,OS_iD'
      '')
    ValidateWithMask = True
    Left = 578
    Top = 288
    object Q_HISTNFI_NUMERO: TStringField
      DisplayLabel = 'Nota Fiscal'
      DisplayWidth = 9
      FieldName = 'NFI_NUMERO'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_NUMERO'
      FixedChar = True
      Size = 10
    end
    object Q_HISTOS_ID: TIntegerField
      Alignment = taCenter
      DisplayLabel = 'OS'
      DisplayWidth = 6
      FieldName = 'OS_ID'
      Origin = 'NBFDADOS.HISTORICO_NF.OS_ID'
    end
    object Q_HISTMANI_ID: TIntegerField
      Alignment = taCenter
      DisplayLabel = 'Manifesto'
      DisplayWidth = 10
      FieldName = 'MANI_ID'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_DEMI'
    end
    object Q_HISTTIPO: TStringField
      Alignment = taCenter
      DisplayLabel = 'Tipo'
      DisplayWidth = 15
      FieldName = 'TIPO'
      Origin = 'NBFDADOS.HISTORICO_NF.TIPO'
      FixedChar = True
    end
    object Q_HISTNFI_DEMI: TDateTimeField
      DisplayLabel = 'Dt.Entrada'
      DisplayWidth = 9
      FieldName = 'NFI_DEMI'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_DEMI'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object Q_HISTNFI_DTSAIDA: TDateTimeField
      DisplayLabel = 'Dt. Entrega'
      DisplayWidth = 9
      FieldName = 'NFI_DTSAIDA'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_DTSAIDA'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object Q_HISTNFI_ENTREGA: TStringField
      DisplayLabel = 'Situação'
      DisplayWidth = 20
      FieldName = 'NFI_ENTREGA'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_ENTREGA'
      FixedChar = True
    end
    object Q_HISTMOT_NOME: TStringField
      DisplayLabel = 'Motorista'
      DisplayWidth = 28
      FieldName = 'MOT_NOME'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_NUMERO'
      FixedChar = True
      Size = 40
    end
    object Q_HISTVEIC_NOME: TStringField
      DisplayLabel = 'Tp. Veículo'
      DisplayWidth = 18
      FieldName = 'VEIC_NOME'
      Origin = 'NBFDADOS.HISTORICO_NF.CLIN_RAZA'
      FixedChar = True
      Size = 30
    end
    object Q_HISTREG_NOME: TStringField
      DisplayLabel = 'Região'
      DisplayWidth = 40
      FieldName = 'REG_NOME'
      FixedChar = True
      Size = 40
    end
    object Q_HISTNFI_QTDEUV: TFloatField
      DisplayLabel = 'Qtde. UV'
      DisplayWidth = 13
      FieldName = 'NFI_QTDEUV'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_QTDEUV'
      DisplayFormat = '0.000'
      EditFormat = '0.000'
    end
    object Q_HISTNFI_PBRU: TFloatField
      DisplayLabel = 'Peso'
      DisplayWidth = 12
      FieldName = 'NFI_PBRU'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_PBRU'
      DisplayFormat = '###,###,##0.0000'
      EditFormat = '###,###,##0.0000'
    end
    object Q_HISTNFI_TOTA: TFloatField
      DisplayLabel = 'Vlr. da Nota'
      DisplayWidth = 12
      FieldName = 'NFI_TOTA'
      Origin = 'NBFDADOS.HISTORICO_NF.NFI_TOTA'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object Q_HISTCLIN_RAZA: TStringField
      FieldName = 'CLIN_RAZA'
      Origin = 'NBFDADOS.HISTORICO_NF.CLIN_RAZA'
      Visible = False
      FixedChar = True
      Size = 50
    end
    object Q_HISTCH: TIntegerField
      FieldName = 'CH'
      Origin = 'NBFDADOS.HISTORICO_NF.CH'
      Visible = False
    end
  end
  object DS_HIST: TwwDataSource
    DataSet = Q_HIST
    Left = 648
    Top = 216
  end
  object Q_AUX: TwwQuery
    DatabaseName = 'NbfDados'
    SessionName = 'Sec_BaseDados'
    ValidateWithMask = True
    Left = 685
    Top = 258
  end
  object dsExcel: TDataSource
    DataSet = Q_HIST
    Left = 264
    Top = 296
  end
end
