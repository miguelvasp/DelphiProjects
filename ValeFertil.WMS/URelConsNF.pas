...
uses
  ...
  gtQRXport_Doc, gtQRXport_RTF, gtQrCtrls, gtQRXport_Excel, jpeg;

type
  TFRelConsNF = class(TForm)
    ConsNF: TQuickRep;
    QRBand1: TQRBand;
    QRShape2: TgtQRShape;
    QRShape3: TgtQRShape;
...
var
  FRelConsNF: TFRelConsNF;
  TOTALUV : real;
  TOTALPESO  : real;
  TOTALVALOR   : real;
  TOTALPALLET : integer ;
implementation
uses UConsultaNF,UFConsultaNF ;
{$R *.DFM}

procedure TFRelConsNF.QRBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
     TOTALVALOR := (TOTALVALOR + FConsultaNF.Q_CONSULNFI_TOTA.asfloat) ;
     TOTALPESO  := (TOTALPESO + FConsultaNF.Q_CONSULPESOBRU.asfloat) ;
     TOTALUV  := (TOTALUV + FConsultaNF.Q_CONSULQTDEUV.asfloat) ;
     TOTALPALLET  := (TOTALPALLET + FConsultaNF.Q_CONSULQTDEPALLET.asinteger) ;
end;

procedure TFRelConsNF.FormCreate(Sender: TObject);
begin
     Q_PAR.open ;
     TOTALVALOR := 0 ;
     TOTALPESO  := 0 ;
  ...
  case Tipo of
    ...
    14 : qrLABEL2.caption := 'Nota Fiscal de Retorno Simbólico' ;
  ...
  IF PROD <> '' then  begin
     qrLabel29.caption := FFConsultaNF.combo_produto.text   ;
     qrLabel30.caption := FFConsultaNF.Q_PRODPRO_DESC.asstring   ;
  end else begin
  ...
  IF REM <> '' then
     qrLabel26.caption :=  FFConsultaNF.wwDBLookupCombo1.text  ;
  IF FFConsultaNF.wwDBLookupCombo1.text = '' then 
     qrLabel26.caption := 'Não Preenchido'    ;

  IF NOTA <> '' then
     qrLabel25.caption :=  FFConsultaNF.dbnf.text
  else
     qrLabel25.caption :='Não Preenchido' ;

  IF CLIF <> '' then
     qrLabel27.caption :=  FFConsultaNF.combo_cliente_final.text
  else
     qrLabel27.caption := 'Não Preenchido'      ;

  IF CLI <> '' then
     qrLabel28.caption :=  FFConsultaNF.combo_cliente.text
  else
     qrLabel28.caption := 'Não Preenchido'    ;

  IF VENCI <> 0.00 then
     qrLabel32.caption :=  FFConsultaNF.DATA1.text
  else
     qrLabel32.caption := 'Não Preenchido'       ;

  IF VENCF <> 0.00 then
     qrLabel33.caption :=  FFConsultaNF.DATA2.text
  else
     qrLabel33.caption := 'Não Preenchido' ;

  IF (TPROD <> '') and (TPROD <> '*')  then
     qrLabel35.caption :=  FFConsultaNF.wwDBLookupCombo2.text
  else
    qrLabel35.caption := 'Não Preenchido' ;

  IF BATCH <> 0.00 then
     qrLabel41.caption :=  FFConsultaNF.MaskEDIT1.text
  else
     qrLabel41.caption := 'Não Preenchido'       ;

  IF BATCHF <> 0.00 then
     qrLabel43.caption :=  FFConsultaNF.MaskEDIt2.text
  else
     qrLabel43.caption := 'Não Preenchido' ;

  IF LOTE <> '' then
     qrLabel45.caption :=  FFConsultaNF.wwdbedit1.text
  else
...
procedure TFRelConsNF.QRBand3AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
     TOTALVALOR := 0 ;
     TOTALPESO  := 0 ;
...
procedure TFRelConsNF.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TFRelConsNF.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    Q_PAR.close ;
    action := cafree ;
    FRelConsNF := nil; 
