...
begin
			...
			Abort;
                ...
                Begin
                        MessageDlg('Este lote já existe!',mtWarning,[MbOk],0);
                        Abort;
                End;
...
begin
               ...
               { IF FReceb.Q_RVCT.IsEmpty then
                BEGIN
                    MessageBox(Self.Handle, 'Não há vencimentos para esta nota, favor verificar!', 'Erro', MB_OK + MB_ICONERROR);
                    Abort;
                end;  
                ...
                //Verificação de consistências, checagem de lançamentos de lotes controlados.
                ...
                //Verficação de valor total das mercadorias
                ...
                If MessageDlg('Deseja realmente dar entrada no Estoque? Note que não será mais possível alterar os itens (Qtde, Preço) após esta confirmação!',
                ...
                Begin
                        ...
                        begin
                             IF Q_AUX.FieldByName('GENERICO').AsString <> 'S' then begin
                               ...
                               begin
                                    MessageBox(Self.Handle, PChar('Lote não informado para este item: '+Q_AUX.FieldByName('mat_desc').AsString), 'Erro', MB_OK + MB_ICONERROR);
                                    Abort;
                               end;
                               if Q_AUX.FieldByName('VALID').AsString = '' then
                               begin
                                    MessageBox(Self.Handle, PChar('Data de vencimento não informada para este item: '+Q_AUX.FieldByName('mat_desc').AsString), 'Erro', MB_OK + MB_ICONERROR);
                                    Abort;
                               end;
                             end;
                           Q_AUX.Next;
                        ...
                        Begin
                           ...
                           IF Q_AUX.FieldByName('GENERICO').AsString <> 'S' then begin
                                ...
                                Begin
                                        ...
                                        SP_AJALM.ExecProc;
                                    //alterada posição de geração de pagamentos
                                     ...
                                     //   SP_GERA_CPAPAGA.ExecProc;
                                        ...
                                        Else Begin
                                                ...
                                                SQL :=  ' UPDATE         REC_LOTE ' +
                                                 ...
                                                 {Miguel: Custo Médio - Cancelada essa trigger agora se usa a procedure}
                                                ...
                                                //Vefifica se é Atendimento Parcial ou Total do item da Ordem de Compra.
                                                ...
                                                //Ordem_Item. Não atender a esta condição significa
                                                ...
                                                //Este SQL Será executado fora do loop.
                                                SQL2 := ' UPDATE        REC_NF ' +
                                                        ' SET           STATUS = ' + '''' + 'F' + '''' +
                        ...
                        Begin
                                ...
                                if MessageBox(Self.Handle, 'Deseja emitir as etiquetas da nota fiscal?', 'Confirmação', MB_YESNO) = 6 then begin
                ...
                //ADICIONA OS VALORES DE CUSTO MEDIO E CUSTO DE REPOSIÇÃO NO ESTOQUE
                WITH Q_AUX DO begin
                    close;
                    SQL.Clear;
                ...
                begin
                        MessageBox(Self.Handle, PChar('A entrada da nota gerou custo médio negativo' + #13 + 'Verificar informações!'), 'Erro', MB_OK + MB_ICONERROR)  ;
                end;


                //CHAMA A TELA DE IMPRESSÃO DE ETIQUETAS
