...
uses
  ...
  StdCtrls, Mask, ExtCtrls, wwdblook, Db, ADODB, ComCtrls, QuickRpt, ComObj,
  Qrctrls;
...
var total, percentual, percentual_soma, somatoria : Real;
    filtro : WideString;
    linha : Integer;
    Excel : Variant;
    i, Coluna : Integer;
begin
 ...
 SomaValor := 0;
  ...
  if rbSaida.ItemIndex = 0 then begin
        QuickRep1.Preview;
  end
  else begin
         ...
         // end;
          ...
          Excel.Cells[Linha, 2] := 'Qtde';
          Excel.Cells[Linha, 3] := 'Valor';
          Excel.Cells[Linha, 4] := 'Percentual';
          Excel.Cells[Linha, 5] := 'Somatoria';

          somatoria := 0;
          qrRel.First;
          while not qrRel.Eof do begin
              Inc(linha);
              Excel.Cells[Linha, 1] := qrRelMAT_DESC.AsString;
              Excel.Cells[Linha, 2] := qrRelQTDE.AsInteger;
              Excel.Cells[Linha, 3] := FormatFloat('##0.00', qrRelVALOR.AsFloat);
              Excel.Cells[Linha, 4] := FormatFloat('##0.00', qrRelPERCENTUAL.AsFloat) ;
              somatoria := somatoria + qrRelPERCENTUAL.AsFloat;
              Excel.Cells[Linha, 5] := FormatFloat('##0.00', somatoria);
