...
begin
        ...
        SQLPrin  := 'SELECT     I.*, ' +
                    ...
                    '    	MAT.UNI_ID, ' +
                    '    	M.MARCA_DESC, ' +
                    '           ROUND(I.QTDE_INV,2) * ROUND(I.REPOSICAO,2) AS VALOR ' +
                    ' FROM      INV I, ' +
                    '    	MATERIAIS MAT, ' +
                    '    	CPAFORN FORN, ' +
                    '    	MARCAS M ' +
...
begin
        ...
        Begin
                MessageDlg('Não será possível gerar inventário porque o usuário ' + Q_AUX.FieldByName('USU_INV').AsString +
                ' está utilizando este recurso, por favor tente dentro de alguns minutos.',mtInformation, [mbOk], 0);
        End Else Begin //Está vazio e pode entrar
                SQL := 'UPDATE          PARAMETROS ' +
                ...
                If MessageDlg('Deseja realmente gerar Inventário?',
...
begin
        ...
        //A rigor StrChave deve ser sempre não nula pois nesta tela não é feito insert.
        If StrChave <> '' then
                Q_INV.Locate('INV_ID',StrChave,[]);
end;
...
begin
        ...
        Begin
                MessageDlg('Não é permitido alterar Número de Lote para registros gerados a partir de saldo em Almoxarifado!',mtInformation, [mbOk], 0);
                abort;
        End;
...
begin
        ...
        End Else Begin
                ...
                End Else Begin
                        If INVX = '' then
                                MessageDlg('Para ver detalhes é necessário haver registro!',mtInformation, [mbOk], 0);
                End;
        End;
end;
...
begin
       ...
       If (Q_AUX.FieldByName('USU_INV').AsString <> '') And
        ...
        End Else Begin //Está vazio e pode entrar
                ...
                If txtInv.Text = '' then
                Begin
                        MessageDlg('Favor filtrar por inventário!',
        ...
        If ckQt.Checked then
                ...
                //Tipo de documento = Inventário "INV"
                SQL :=  ' SELECT        TD_ID '+
                        ' FROM          TIPO_DOCUMENTO ' +
                        ' WHERE         APELIDO = ' + '''' + 'INV' + '''';
        ...
        If ckQt.Checked then
                ...
                Begin
                        MessageDlg('Antes é necessário preencher todas as quantidades para este o Inventário!',
                                mtInformation, [mbOk], 0);
                        abort;
                End Else Begin
                        ...
                        If MessageDlg('Deseja realmente atualizar almoxarifado? Só serão atualizados os registos que aparecem no grid e que tenham status "Aberto" ou "Com Erro"!',
                        mtInformation, [mbYes, mbNo], 0) = mrYes then
                        Begin
                                Q_INV.First;
                                ...
                                Begin
                                        //Permite que gere atualização apenas quem tiver status
                                        ...
                                        //Porque pode ser que não gere ajuste mas sempre tem que indicar que já foi considerado (gerado)
                                        Begin
                                                SQL :=  'UPDATE         INV ' +
                                                        'SET            STATUS = ' + '''' + 'G' + '''' +
                                                        'WHERE          INV_ID = ' + Q_INVInv_ID.AsString;
                                ...
                                If Fez then
                                Begin
                                        MessageDlg('Inventário Executado!',
                                        mtInformation, [mbOk], 0);
...
Begin
        ...
        IF Ninv = '' THEN BEGIN
                MessageDlg(' Favor preencher o número de Inventário !',mtInformation, [MbOk], 0);
                txtInv.SetFocus;
                abort;
        End Else Begin
                ...
                End Else Begin
                        ...
                        MessageDlg('Este número de Inventário não existe!',mtInformation, [mbOk], 0);
                End;
        End;
End;
...
begin
     ...
     If NINV = '' Then Begin
                MessageDlg(' Num. Inventário, preenchimento obrigatorio! ',mtInformation, [MbOk], 0);
                txtInv.SetFocus;
     End Else Begin
               INVX := '0';
...
begin
    ...
    Else Begin
        ...
        If MessageDlg('Deseja realmente apagar o inventário ' + NINV + '?',
           mtInformation, [mbYes, mbNo], 0) = mrYes then
        Begin
            If MessageDlg('Lembre-se os dados do inventário ' + NINV + ' serão excluidos do Banco de Dados! ',
               mtInformation, [mbYes, mbNo], 0) = mrYes then
            Begin
...
begin
      IF txtInv.Text = '' THEN BEGIN
              messagebox(0,'Favor Selecionar um Num. Inventário !!!','Vetorpel',(mb_Ok + Mb_IconInformation + 8192));
              txtInv.SetFocus;
              abort ;
      END;
      ...
      IF Data1.Text = '  /  /     :  ' THEN BEGIN
              messagebox(0,'Favor Selecionar o Período !!!','Vetorpel',(mb_Ok + Mb_IconInformation + 8192));
              Data1.SetFocus;
              abort ;
      END;
     ...
     NINV:= txtInv.Text;
      ...
      If MessageDlg('Deseja realmente alterar todas as datas do inventário Num.'  + NINV + ' ? ' ,
                        mtInformation, [mbYes, mbNo], 0) = mrYes then
      Begin
...
begin
      ...
      If  Q_INVStatus.AsString <> 'G' Then Begin
          ...
          If MessageDlg('Deseja realmente alterar todos os Custos Médios do inventário Num.'  + txtInv.Text + ' e do Material: ' + Q_INVMat_Desc.AsString + ' ? ' ,
                        mtInformation, [mbYes, mbNo], 0) = mrYes then
          Begin
                ...
                messagebox(0,'Favor selecionar um novo Custo Médio !!!','Vetorpel',(mb_Ok + Mb_IconInformation + 8192));
                Edit1.SetFocus;
                abort ;
     ...
     Else Begin
        messagebox(0,'Este Inventário esta com Status "Gerado", não é possível alterar!','Vetorpel',(mb_Ok + Mb_IconInformation + 8192));
        txtInv.SetFocus;
        abort ;
...
begin
  ...
  Excel.Cells[Linha, 5] :=  'Validade';
  Excel.Cells[Linha, 6] :=  'Qtde Almox';
  Excel.Cells[Linha, 7] :=  'Inventário';
  Excel.Cells[Linha, 8] :=  'Qtde dif';
  Excel.Cells[Linha, 9] :=  'Valor rep';
  Excel.Cells[Linha, 10] :=  'Total';

  Q_INV.First;
  while not Q_INV.Eof do begin
    Inc(linha);
      Excel.Cells[Linha, 1] :=  Q_INVMAT_DESC.AsString;
      Excel.Cells[Linha, 2] :=  Q_INVN_Lote.AsString;
      Excel.Cells[Linha, 3] :=  Q_INVFor_Codi.AsString ;
      Excel.Cells[Linha, 4] :=  Q_INVMARCA_DESC.AsString;
      Excel.Cells[Linha, 5] :=  Q_INVValid.AsDateTime;
      Excel.Cells[Linha, 6] :=  Q_INVQtde_Almox.AsFloat;
      Excel.Cells[Linha, 7] :=  Q_INVQtde_Inv.AsFloat;
      if q_invQtde_Inv.AsString <> '' then begin
          Excel.Cells[Linha, 8] :=  Q_INVQtde_Almox.AsFloat - Q_INVQtde_Inv.AsFloat;
          qrAux.Close;
          qrAux.Params[0].Value := Q_INVMat_ID.AsInteger;
          qrAux.Open;
          Excel.Cells[Linha, 9] :=  FormatFloat('###,###,##0.00', qrAuxREPOSICAO.AsFloat);
          Excel.Cells[Linha, 10] :=  FormatFloat('###,###,##0.00', qrAuxREPOSICAO.AsFloat * Q_INVQtde_Almox.AsFloat - Q_INVQtde_Inv.AsFloat);
      end;
      Q_INV.Next;
  end;
