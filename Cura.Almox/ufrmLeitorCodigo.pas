...
begin
    ...
    if key = vk_return then begin
        ...
        if qrBarras.IsEmpty then begin
           ...
           if Trim(UpperCase(qrAux.FieldByName('N_Lote').AsString)) <> Trim(UpperCase(qrBarrasN_LOTE.AsString)) then begin
             if  Application.MessageBox(pchar('O lote selecionado ' + qrBarrasN_LOTE.AsString + ' - Venc: ' + formatdatetime('dd/MM/YYYY', qrBarrasVALIDADE.AsDateTime) +  ' não é o mais antigo, o lote ' +  qrAux.FieldByName('N_Lote').AsString + ' - Venc:' + formatdatetime('dd/MM/YYYY', qrAux.FieldByName('Valid').AsDateTime) + ' é o mais antigo do estoque ' +  #13#10 + 'Deseja usar este lote mesmo assim?'),
