object frmEstoque_minimo: TfrmEstoque_minimo
  Left = 606
  Top = 673
  ActiveControl = cboMaterial
  BorderStyle = bsSingle
  Caption = 'Consulta de Itens Abaixo do Estoque Minimo'
  ClientHeight = 662
  ClientWidth = 984
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = [fsBold]
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object RzPageControl1: TRzPageControl
    Left = 0
    Top = 0
    Width = 984
    Height = 204
    ActivePage = TabSheet1
    Align = alTop
    TabIndex = 0
    TabOrder = 0
    FixedDimension = 19
    object TabSheet1: TRzTabSheet
      Caption = 'Filtros'
      object Label1: TLabel
        Left = 26
        Top = 16
        Width = 57
        Height = 13
        Caption = 'Material:'
      end
      object Label2: TLabel
        Left = 40
        Top = 53
        Width = 43
        Height = 13
        Caption = 'Marca:'
      end
      object Label3: TLabel
        Left = 40
        Top = 87
        Width = 43
        Height = 13
        Caption = 'Grupo:'
      end
      object Label4: TLabel
        Left = 6
        Top = 124
        Width = 79
        Height = 13
        Caption = 'Fornecedor:'
      end
      object Label5: TLabel
        Left = 54
        Top = 156
        Width = 32
        Height = 13
        Caption = 'Tipo:'
      end
      object RzRadioGroup1: TRzRadioGroup
        Left = 792
        Top = -2
        Width = 177
        Height = 56
        Anchors = [akTop, akRight]
        Caption = 'Selecione o Tipo'
        ItemFont.Charset = ANSI_CHARSET
        ItemFont.Color = clWindowText
        ItemFont.Height = -11
        ItemFont.Name = 'Verdana'
        ItemFont.Style = []
        ItemIndex = 1
        Items.Strings = (
          'Todos'
          'Itens Abaixo do Estoque')
        TabOrder = 0
      end
      object cboMarca: TwwDBLookupCombo
        Left = 88
        Top = 48
        Width = 681
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'MARCA_DESC'#9'20'#9'MARCA'#9'F')
        LookupTable = Q_MARCA
        LookupField = 'MARCA_ID'
        TabOrder = 2
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        OnExit = cboMarcaExit
      end
      object cboMaterial: TwwDBLookupCombo
        Left = 88
        Top = 12
        Width = 649
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'MAT_DESC'#9'60'#9'MATERIAL'#9'F')
        LookupTable = Q_MAT
        LookupField = 'MAT_ID'
        TabOrder = 1
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        OnExit = cboMaterialExit
      end
      object cboGrupo: TwwDBLookupCombo
        Left = 88
        Top = 84
        Width = 681
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'GRU_DESC'#9'40'#9'GRUPO'#9'F')
        LookupTable = Q_GRUPO
        LookupField = 'GRU_ID'
        TabOrder = 4
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        OnExit = cboGrupoExit
      end
      object cbofornecedor: TwwDBLookupCombo
        Left = 88
        Top = 120
        Width = 681
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'FOR_RAZA'#9'50'#9'FORNECEDOR'#9'F')
        LookupTable = Q_FORNECEDOR
        LookupField = 'FOR_CODI'
        TabOrder = 6
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        OnExit = cbofornecedorExit
      end
      object BitBtn1: TBitBtn
        Left = 792
        Top = 56
        Width = 177
        Height = 28
        Anchors = [akTop, akRight]
        Caption = 'Pesquisar'
        TabOrder = 3
        OnClick = BitBtn1Click
        Glyph.Data = {
          
          4242424242424242424242424242424242424242424242424242}
      end
      object BitBtn2: TBitBtn
        Left = 792
        Top = 86
        Width = 177
        Height = 28
        Anchors = [akTop, akRight]
        Caption = 'Relatório'
        TabOrder = 5
        OnClick = BitBtn2Click
        Glyph.Data = {
          
          8282828282828282826A61646464686E6E828282828282828282}
      end
      object BitBtn3: TBitBtn
        Left = 792
        Top = 114
        Width = 177
        Height = 29
        Anchors = [akTop, akRight]
        Caption = 'Limpar Filtros'
        TabOrder = 7
        OnClick = BitBtn3Click
        Glyph.Data = {
          
          D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4C8D0D4}
      end
      object chk1: TCheckBox
        Left = 512
        Top = 152
        Width = 257
        Height = 17
        Caption = 'Somente materiais críticos'
        TabOrder = 8
      end
      object cboTipoMaterial: TComboBox
        Left = 88
        Top = 152
        Width = 385
        Height = 21
        Style = csDropDownList
        ItemHeight = 13
        TabOrder = 9
        Items.Strings = (
          'Todos'
          'Desconsiderar materiais do Laboratório'
          'Somente materiais do Laboratório')
      end
      object BitBtn4: TBitBtn
        Left = 792
        Top = 146
        Width = 177
        Height = 29
        Anchors = [akTop, akRight]
        Caption = 'Gerar Sol. Compras'
        TabOrder = 10
        OnClick = BitBtn4Click
      end
      object btnGetMaterial: TBitBtn
        Left = 746
        Top = 9
        Width = 32
        Height = 28
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 11
        OnClick = btnGetMaterialClick
        Glyph.Data = {
          
          4242424242424242424242424242424242424242424242424242}
      end
    end
  end
  object RzPageControl2: TRzPageControl
    Left = 0
    Top = 204
    Width = 984
    Height = 458
    ActivePage = TabSheet2
    Align = alClient
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Verdana'
    Font.Style = []
    ParentFont = False
    TabIndex = 0
    TabOrder = 1
    FixedDimension = 19
    object TabSheet2: TRzTabSheet
      Caption = 'Resultado da Pesquisa'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      object wwDBGrid1: TwwDBGrid
        Left = 0
        Top = 0
        Width = 980
        Height = 435
        ControlType.Strings = (
          'SEL;CheckBox;S;N')
        Selected.Strings = (
          'MATERIAL'#9'40'#9'MATERIAL'#9#9
          'GRUPO'#9'25'#9'GRUPO'#9#9
          'ESTOQUE'#9'8'#9'ESTOQUE'#9#9
          'MEDIA'#9'8'#9'MEDIA'#9#9
          'ESTOQUE_MINIMO'#9'10'#9'ESTOQUE MÍNIMO'#9#9
          'OC'#9'10'#9'QTDE EM O.C.'#9#9
          'UN'#9'2'#9'UN'#9#9
          'SEL'#9'1'#9'***'#9#9
          'QTDESOLICITADA'#9'10'#9'Qtde. Comprar'#9#9)
        IniAttributes.Delimiter = ';;'
        TitleColor = clBtnFace
        FixedCols = 7
        ShowHorzScrollBar = True
        Align = alClient
        DataSource = DataSource1
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        TitleAlignment = taLeftJustify
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clNavy
        TitleFont.Height = -11
        TitleFont.Name = 'Verdana'
        TitleFont.Style = []
        TitleLines = 1
        TitleButtons = False
        OnCalcCellColors = wwDBGrid1CalcCellColors
      end
    end
  end
  object Q_MARCA: TwwQuery
    CachedUpdates = True
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM MARCAS'
      'ORDER BY MARCA_DESC')
    ValidateWithMask = True
    Left = 552
    Top = 472
    object Q_MARCAMARCA_DESC: TStringField
      DisplayLabel = 'MARCA'
      DisplayWidth = 20
      FieldName = 'MARCA_DESC'
      Origin = 'CURA.MARCAS.MARCA_DESC'
      FixedChar = True
    end
    object Q_MARCAMARCA_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'MARCA_ID'
      Origin = 'CURA.MARCAS.MARCA_ID'
      Visible = False
    end
  end
  object Q_GRUPO: TwwQuery
    CachedUpdates = True
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM GRUPO_MATERIAL'
      'ORDER BY GRU_DESC')
    ValidateWithMask = True
    Left = 416
    Top = 456
    object Q_GRUPOGRU_DESC: TStringField
      DisplayLabel = 'GRUPO'
      DisplayWidth = 40
      FieldName = 'GRU_DESC'
      Origin = 'CURA.GRUPO_MATERIAL.GRU_DESC'
      FixedChar = True
      Size = 40
    end
    object Q_GRUPOGRU_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'GRU_ID'
      Origin = 'CURA.GRUPO_MATERIAL.GRU_ID'
      Visible = False
    end
  end
  object Q_FORNECEDOR: TwwQuery
    CachedUpdates = True
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT FOR_CODI, FOR_RAZA FROM CPAFORN'
      'WHERE F_ALMOX = '#39'S'#39
      'ORDER BY FOR_RAZA')
    ValidateWithMask = True
    Left = 264
    Top = 416
    object Q_FORNECEDORFOR_RAZA: TStringField
      DisplayLabel = 'FORNECEDOR'
      DisplayWidth = 50
      FieldName = 'FOR_RAZA'
      Origin = 'CURA.CPAFORN.FOR_RAZA'
      FixedChar = True
      Size = 50
    end
    object Q_FORNECEDORFOR_CODI: TStringField
      DisplayWidth = 15
      FieldName = 'FOR_CODI'
      Origin = 'CURA.CPAFORN.FOR_CODI'
      Visible = False
      FixedChar = True
      Size = 15
    end
  end
  object qrExecProc: TwwQuery
    CachedUpdates = True
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      
        'exec REL_ESTOQUE_MINIMO :material, :marca, :grupo, :fornecedor, ' +
        ':tipo, :CRITICO, :usuario, :tipo')
    ValidateWithMask = True
    Left = 680
    Top = 496
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'material'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'marca'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'grupo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fornecedor'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tipo'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CRITICO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'usuario'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'tipo'
        ParamType = ptUnknown
      end>
  end
  object DataSource1: TDataSource
    DataSet = qrConsulta
    Left = 137
    Top = 413
  end
  object qrConsulta: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'USUARIO'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * FROM ESTOQUE_MINIMO'
      'WHERE ESTOQUE_MINIMO > 0'
      'AND USUARIO = :USUARIO'
      'ORDER BY MATERIAL')
    Left = 473
    Top = 336
    object qrConsultaMATERIAL: TStringField
      DisplayWidth = 40
      FieldName = 'MATERIAL'
      ProviderFlags = []
      Size = 60
    end
    object qrConsultaGRUPO: TStringField
      DisplayWidth = 25
      FieldName = 'GRUPO'
      ProviderFlags = []
      Size = 40
    end
    object qrConsultaESTOQUE: TFloatField
      DisplayWidth = 8
      FieldName = 'ESTOQUE'
      ProviderFlags = []
    end
    object qrConsultaMEDIA: TFloatField
      DisplayWidth = 8
      FieldName = 'MEDIA'
      ProviderFlags = []
    end
    object qrConsultaESTOQUE_MINIMO: TFloatField
      DisplayLabel = 'ESTOQUE MÍNIMO'
      DisplayWidth = 10
      FieldName = 'ESTOQUE_MINIMO'
      ProviderFlags = []
    end
    object qrConsultaOC: TFloatField
      DisplayLabel = 'QTDE EM O.C.'
      DisplayWidth = 10
      FieldName = 'OC'
      ProviderFlags = []
    end
    object qrConsultaUN: TStringField
      DisplayWidth = 2
      FieldName = 'UN'
      ProviderFlags = []
      Size = 2
    end
    object qrConsultaSEL: TStringField
      DisplayLabel = '***'
      DisplayWidth = 1
      FieldName = 'SEL'
      ProviderFlags = [pfInUpdate]
      Size = 1
    end
    object qrConsultaQTDESOLICITADA: TFloatField
      DisplayLabel = 'Qtde. Comprar'
      DisplayWidth = 10
      FieldName = 'QTDESOLICITADA'
      ProviderFlags = [pfInUpdate]
    end
    object qrConsultaCRITICO: TStringField
      FieldName = 'CRITICO'
      ProviderFlags = []
      Visible = False
      Size = 1
    end
    object qrConsultaMAT_ID: TIntegerField
      FieldName = 'MAT_ID'
      ProviderFlags = []
      Visible = False
    end
    object qrConsultaID: TAutoIncField
      FieldName = 'ID'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
      Visible = False
    end
    object qrConsultaUSUARIO: TIntegerField
      FieldName = 'USUARIO'
      ProviderFlags = []
      Visible = False
    end
  end
  object Q_MAT: TADOQuery
    Connection = FMenu.ADOConnection
    Parameters = <>
    SQL.Strings = (
      
        'Select MAT_ID, CodigoMaterial + '#39' '#39' + MAT_DESC AS MAT_DESC FROM ' +
        'MATERIAIS'
      'WHERE (GENERICO <> '#39'S'#39' OR GENERICO IS NULL)'
      'AND MAT_ATIVO = '#39'S'#39
      'ORDER BY MAT_DESC')
    Left = 209
    Top = 288
    object Q_MATMAT_ID: TAutoIncField
      FieldName = 'MAT_ID'
      ReadOnly = True
    end
    object Q_MATMAT_DESC: TStringField
      FieldName = 'MAT_DESC'
      ReadOnly = True
      Size = 151
    end
  end
end
