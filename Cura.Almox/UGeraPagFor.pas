...
begin
    ...
    // Inc(TotalLotesCNAB);
     ...
     begin
         ...
         L := L + writeString('01', 2); //Codigo de operação transferencia entre contas
         L := L + writeString('040', 3);//VERSAO DO LAYOUT
         L := L + writeString('', 1); //BRANCO
         L := L + writeString('2', 1); //TTIPO DE INSCRIÇÃO = CNPJ
         L := L + writeString(qrEmpresaCNPJ.AsString, 14);  //CNPJ
         // L := L + writeString('1707', 4);
         L := L + writeString('', 4); //IDENTIFICAÇÃO
         ...
         //COMEÇAMOS A INFORMAR A MOVIMENTAÇÃO DO LOTE
         ...
         WHILE not qrItau.Eof DO BEGIN
             ...
             L := L + writeString('', 8); //identificação da instituição para o SPB
             L := L + writeInt(0, 7);
             ...
             L := L + writeString('00000000', 8);  //data de efetivação real 
...
//traz os pagamentos do tipo TED =
     ...
     begin
         ...
         L := L + writeInt(LoteAtual, 4);
         L := L + writeString('1', 1);
         L := L + writeString('C', 1); //TIPO DE OPERAÇÃO
         L := L + writeString('20', 2);  //TIPO DE PAGAMENTO
         ...
         WHILE not qrItau.Eof DO BEGIN
             ...
             //começa a ler o codigo de barras
             ...
             L := L + writeString('J', 1); //codigo do segmento
             L := L + writeString('000', 3); //tipo de movimento
             L := L + writeString('52', 2); //CÓDIGO DO REGISTRO
             L := L + '2'; //tipo inscrição
     ...
     //****************************************************************************
     //Boletos bancarios de bancos que não o itau
     //****************************************************************************
...
Begin
        Sair := False;
        Mult := 1;
        // O primeiro campo tem 9 posições, os outros dois têm 10
        TamCampo := 9;

        Total :=  0;
        TextoCompleto := Copy(Txt,1,TamText);
        //Só seleciona até o campo 3, a partir do dígito geral não faz calculo de dígito de controle
...
begin
    ...
    Linha := Linha + Funcoes.writeString(qrEmpresaCNPJ.AsString,14); //inscrição
    Linha := Linha + Funcoes.writeString(qrEmpresacodigo_pe.AsString,20);//codigo de autorização
    ...
    Linha := Linha + Funcoes.writeInt(1, 6); //criar auto numeração
    ...
    if qrUnibanco.IsEmpty = False then begin
              ...
              Linha := Linha + 'C';//tipo de operação
              Linha := Linha + '98';//tipo de serviço - pagamentos diversos
              Linha := Linha + '41';//forma de lançamento, verificar com Rubens
              Linha := Linha + '031';//versao do layout
              Linha := Linha + Funcoes.writeString('', 1);//cnab em branco
              Linha := Linha + '2';//tipo = cnpj
              Linha := Linha + Funcoes.writeStringZero(qrEmpresaCNPJ.AsString,14); //inscrição
              Linha := Linha + Funcoes.writeString(qrEmpresacodigo_pe.AsString,20);//codigo de autorização
              Linha := Linha + Funcoes.writeInt(qrEmpresaAgencia_PE.AsInteger, 5);//agencia do pagador
              if qrEmpresadgAgencia_PE.AsString <> '' then
                  Linha := Linha + Funcoes.writeString(qrEmpresadgAgencia_PE.AsString, 1)//digito agencia do pagador
              ...
              while not qrUnibanco.Eof do begin
                        ...
                        Linha := Linha + FormatDateTime('ddMMyyyy', date);//data real para efetivação
                        Linha := Linha + Funcoes.writeFloat(qrUnibancoValor.AsFloat,15,2,TRUE);
    ...
    if qrUnibanco.IsEmpty = False then begin
              ...
              Linha := Linha + 'C';//tipo de operação
              Linha := Linha + '98';//tipo de serviço - pagamentos diversos
              Linha := Linha + '03';//forma de lançamento, verificar com Rubens
              Linha := Linha + '031';//versao do layout
              Linha := Linha + Funcoes.writeString('', 1);//cnab em branco
              Linha := Linha + '2';//tipo = cnpj
              Linha := Linha + Funcoes.writeString(qrEmpresaCNPJ.Value,14); //inscrição
              Linha := Linha + Funcoes.writeString(qrEmpresacodigo_pe.AsString,20);//codigo de autorização
              Linha := Linha + Funcoes.writeInt(qrEmpresaAgencia_PE.AsInteger, 5);//agencia do pagador
              Linha := Linha + Funcoes.writeString(qrEmpresadgAgencia_PE.AsString, 1);//digito agencia do pagador
              Linha := Linha + Funcoes.writeInt(qrEmpresaConta_PE.AsInteger, 12);   //conta corrente
    ...
    if qrUnibanco.IsEmpty = False then begin
              ...
              Linha := Linha + 'C';//tipo de operação
              Linha := Linha + '98';//tipo de serviço - pagamentos diversos
              Linha := Linha + '01';//forma de lançamento, verificar com Rubens
              Linha := Linha + '031';//versao do layout
              Linha := Linha + Funcoes.writeString('', 1);//cnab em branco
              Linha := Linha + '2';//tipo = cnpj
              Linha := Linha + Funcoes.writeString(qrEmpresaCNPJ.Value,14); //inscrição
    ...
    if qrUnibanco.IsEmpty = False then begin
              ...
              Linha := Linha + 'C';//tipo de operação
              Linha := Linha + '98';//tipo de serviço - pagamentos diversos
              //forma de lançamento, titulos do unibanco ou outro banco
...
//Trailer   do arquivo
    ...
    Linha := Linha + Funcoes.writeInt(nTipo1,6);//qtde registros tipo 1 e operação tipo E
   ...
   begin
     MessageBox(Self.Handle, 'Não foi informado destino do arquivo gerado!!!', 'Erro', MB_OK + MB_ICONERROR);
     Abort;
   end;
...
BEGIN
    ...
    qrTitulosReal.Open;
            ...
            {codigo do registro - Tipo de Operação - Sigla da Legenda - FILLER}
            Linha := Linha + '4IPG   ';
           ...
           //atualização da numeração do titulo na tabela cpacheq
...
begin
   ...
   try
      ...
      {Busca das informações sobre conta bancaria}
      qrDadosBancarios.close;
      qrDadosBancarios.Open;

      
      Linha := Linha + '0';{Código do registro - 1}
      Linha := Linha + '1';{Codigo da Remessa - 1}
      Linha := Linha + 'REMESSA';{Literal da Remessa 7}
      Linha := Linha + '05';{Codigo do Serviço - 2}
      Linha := Linha + 'PG FORNECEDORES'; {Literal do Serviço - 15}
      ...
      Linha := Linha + FormatDateTime('DDMMYY',Date); {DATA MOVIMENTAÇÃO}
      Linha := Linha + FormatDateTime('hhmmss',Now);  {HORA DE PROCESSAMENTO}
      Linha := Linha + '000000';  {Nº GERAÇÃO MOVIMENTO - 6}
      ...
      Linha := Linha + FormatFloat('000000',Seq);  {Nº da sequencia}
      ...
      //**************** 5.2 - Registro Trasação Pagamento - Contas Bancarias********************//
      Linha := '';

      {iNICIO do loop de geração de pagamentos}
      ...
      begin
            ...
            {codigo de liberação - 2 nao liberado}
            Linha := Linha + '2';
            ...
            {Código de finalidade padrao 398}
            linha := Linha + '398';
            ...
            {cOD FORMA DE PAGAMENTO - verifica se o pagamento será efetuado no banco real ou em
            ...
            {ind. n/numero cobrança}
            Linha := Linha + '0';

            {INFORMAÇÃO DO CLIENTE 2}
            Linha := Linha + '00000000000';

            {INFORMAÇÃO DO CLIENTE 3}
            Linha := Linha + '0000000';
            ...
            {NUMERO DO DOCUMENTO - COD. TIPO DE COMUNICAÇÃO- CODIGO MODO DE ENTRADA}
            Linha := Linha + '          ';
...
var
caminho   : string;
Valor     : STRING;  // dá o formato ao valor
...
begin
   ...
   {
        layout atualizado para a versão 080 de agosto de 2014 - Efetuada por Miguel em março de 2019
   }
   ...
   BEGIN
        ContadorArq := 1;  //Conta a partir do header de arquivo. Por este motivo não começa com zero
        ValortotArq := 0;
        TP_TRANSANT := '';
        ...
        Begin
            ...
            //Muda quando muda o tipo de Transacao, DOC, TED, etc, que é
            //o fator de ordenação da query.
            If TP_TRANSANT <> Q_TP10TP_TRANS.AsString then
            Begin

                //Terminou um lote e começa outro, assim, escreve o trailer
                If not PrimeiraVez then
                Begin
                      //Registro Trailer de Lote - CHEQUES
                      Inc(ContadorArq);
                      //Seguindo instrução do Wilson de que o contador
                      ...
                      valor_pon:= ValorTot * 100; //Mudei para não usar mais a outra query -- Daniel
                ...
                escrevetexto('C',1);//04   //CRÉDITO
                escrevenumero(TP_PAGTO,2);//05    //DIVERSOS

                //escrevenumero(CHEQ_PAGTO,2);//06 //AQUI TEM QUE PERGUNTAR PRO ITAÚ SE PARA CADA LOTE É UM
                escrevenumero(Q_TP10TP_TRANS.AsInteger,2);

                escrevenumero(40,3);//07
                escrevetexto('',1);//08
                escrevenumero(2,1);//09 --Tipo de inscrição CNPJ, 1 seria CPF.
            ...
            //Variáveis que serão utilizadas no Trailer de Arquivo
            //para totalização de Registros e de Valor
            Inc(ContadorArq);
            ...
            ///Preparam-se as variáveis, separando agência e conta de DAC
            ...
            //Se o banco for Itaú, o formato é um, se for
            ...
            Begin
                    ...
                    escrevenumero(StrtoInt(CC),5);//09 E   --garantir que não tem dac
...
//            escrevetexto(formatdatetime('DDMMYYYY',Now),8);//12
            ...
            //Se for DOC ou TED, é obrigatório o preenchimento do cnpj ou cpf
            //Caso contrario o valor no banco de dados pode ser em branco que será preenchido
            //com zeros, o mesmo vale para Isentos.

            escrevecnpj(Q_TP10FOR_CGC.asstring,14);//23
            escrevetexto('',2);//24
            escrevetexto('00010',5);//25 -- Código de Crédito em Conta (campo finalidade da TED)
            escrevetexto('',5);//26
            escrevetexto('0',1);//27
            escrevetexto('',10);//28
    ...
    BEGIN
        ...
        Begin
            ...
            //Muda quando muda o tipo de Transacao, que pode ser pagamento de título dentro
            //do Itaú ou de outro banco, esta coluna é o fator de ordenação da query.

            If TP_TRANSANT <> Q_TP10TP_TRANS.AsString then
            Begin
                If not PrimeiraVez then
                Begin
                      //Registro Trailer de Lote   - Título
                      Inc(ContadorArq);
                      //Seguindo instrução do Wilson de que o contador
                      //do trailer do arquivo deve totalizar a quantidade de linhas do arquivo.
                      escrevenumero(Q_PARABAN_CODI.AsInteger,3);//01
                ...
                //Seguindo instrução do Wilson de que o contador
                //do trailer do arquivo deve totalizar a quantidade de linhas do arquivo.
                //Registro Header de Lote - LIQUIDAÇÃO DE TITULOS
                ...
                //Para títulos existem duas opções: 30, pagamento de títulos em cobrança no Itaú e
                //                                  31, pagamento de títulos em cobrança em outros bancos
                //AQUI TEM QUE PERGUNTAR PRO ITAÚ SE PARA CADA LOTE É UM

                If (Q_TP10TP_TRANS.AsString = '30') or (Q_TP10TP_TRANS.AsString = '31') then
                        escrevenumero(Q_TP10TP_TRANS.AsInteger,2)//06   //PAGAMENTO DE TÍTULOS
                Else
                        escrevenumero(31,2);//06   //PAGAMENTO DE TÍTULOS

                escrevenumero(30,3);//07
                escrevetexto('',1);//08
                escrevenumero(2,1);//09  --Está fixo como CNPJ
                escrevecnpj(Q_PARACNPJ.asstring,14);//10
                escrevetexto('',20);//11
                escrevenumero(Q_PARABAN_AGEN.AsInteger,5);//12
                escrevetexto('',1);//13
            ...
            IF Q_TP10Barras.ASSTRING <> '' THEN BEGIN
                //Formulado para atender a digitação manual do código de barras.
                //Caso a leitura passe a ser feita "leitor" deverá mudar o lay-out em
                //acordo com a documentação do Itaú para Sispag.
...
//                Barras := CalculaDigitoGeral(Barras);
               ...
               //-- antes destava ao invés de 6,4; 7,4.
            //Código de barras ---------------------------XX
            END ELSE BEGIN
...
//           escrevetexto(formatdatetime('DDMMYYYY',Now),8);//18
   ...
   {/************** miguel *************/}
   {
       comentada por miguel, procedimento executado na tele de programação de pagamentos
   ...
   {/************** end ****************/}
     ...
     //Fim do Trailer de Arquivo, só no fim do arquivo se coloca 10 + 13.
     Write(F,#10+#13);
     CloseFile(F);
...
begin
  ...
  try
       ...
       lbProgress.Caption := 'Processando Informações';
       Gauge1.Visible := True;
       Gauge1.Progress := 0;
       Application.ProcessMessages;
...
{========================Header================================================}
      ...
      //codigo de comunicação fornecido pelo bradesco
...
{================================= PAGAMENTOS =================================}
      ...
      else  //verifica se o titulo é do bradesco ou nao
      begin
        if qrPagBradescoTITULO_BRADESCO.AsString = 'S' then
          ...
          //numero do documento verificar se é para colocar o numero do CP
          Aux :=  '0';
          Size := LENGTH(AUX);
          for I := 1 TO 10 - Size do
              Aux := '0' + Aux;
          ...
          //data para efetivação do pagamento
          Linha := Linha + '00000000';
...
begin
    ...
    // Inc(TotalLotesCNAB);
     ...
     begin
         ...
         // SE FOR CODIGO DE BARRAS VERIFICA SE O TITULO É DO ITAU OU NAO
         ...
         L := L + writeString('2', 1); //TTIPO DE INSCRIÇÃO = CNPJ
         L := L + writeString(qrEmpresaCNPJ.AsString, 14);  //CNPJ
         L := L + writeString('', 20); //IDENTIFICAÇÃO
         L := L + writeINT(qrEmpresaAgencia_PE.AsInteger, 5); //AGENCIA A DEBITAR DINHEIRO
         L := L + writeString('', 1); //BRANCO
         L := L + writeINT(qrEmpresaConta_PE.AsInteger, 12); //CONTA A SER DEBITADA
         L := L + writeINT(qrEmpresadgConta_PE.AsInteger, 1); //DAC DA CONTA
