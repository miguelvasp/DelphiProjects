object FPrepInv: TFPrepInv
  Left = 668
  Top = 238
  AutoScroll = False
  BorderIcons = []
  Caption = 'Inventário - Sistema de Almoxarifado'
  ClientHeight = 576
  ClientWidth = 966
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 966
    Height = 41
    Align = alTop
    BevelInner = bvLowered
    TabOrder = 0
    object SpeedButton3: TSpeedButton
      Left = 882
      Top = 6
      Width = 78
      Height = 30
      Hint = 'Sair'
      Anchors = [akTop, akRight]
      Caption = 'Fechar'
      Glyph.Data = {
        
        EBF2F4EDF3F6F3F7FAFBFCFEFFFFFFFFFFFFFFFFFFFFFFFF0000}
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton3Click
    end
    object Bt_Incluir: TSpeedButton
      Left = 347
      Top = 6
      Width = 73
      Height = 30
      Hint = 'Incluir'
      Caption = 'Incluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      OnClick = Bt_IncluirClick
    end
    object BitBtn20: TBitBtn
      Left = 217
      Top = 6
      Width = 122
      Height = 30
      Hint = 'Busca Integrada'
      Caption = 'Busca Integrada'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = BitBtn20Click
      Glyph.Data = {
        
        4242424242424242424242424242424242424242424242424242}
    end
    object BitBtn1: TBitBtn
      Left = 89
      Top = 6
      Width = 120
      Height = 30
      Hint = 'Apagar Inventário...'
      Caption = 'Apagar Inventário...'
      Enabled = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D}
    end
    object BitBtn2: TBitBtn
      Left = 5
      Top = 6
      Width = 79
      Height = 30
      Hint = 'Ajustes de Almoxarifado'
      Caption = 'Ajuste'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object Pnl1: TPanel
    Left = 0
    Top = 41
    Width = 966
    Height = 535
    Align = alClient
    BevelInner = bvLowered
    TabOrder = 1
    object Grid: TwwDBGrid
      Left = 10
      Top = 183
      Width = 952
      Height = 237
      Selected.Strings = (
        'Num_Inv'#9'6'#9'Inv.'#9#9
        'For_Codi'#9'15'#9'Fornecedor'#9#9
        'MAT_DESC'#9'24'#9'Material'#9#9
        'MARCA_DESC'#9'10'#9'Marca'#9#9
        'N_Lote'#9'8'#9'Lote'#9#9
        'Qtde_Almox'#9'10'#9'Qtde. Almox.'#9'F'#9
        'Qtde_Inv'#9'10'#9'Qtde. Inv.'#9'F'#9
        'Valid'#9'10'#9'Valid'#9#9
        'QtdeCalc'#9'10'#9'Qtde Mov.'#9#9
        'Data'#9'9'#9'Data'#9#9
        'Tipo'#9'9'#9'Tipo'#9#9
        'Usuario'#9'10'#9'Usuário Inv.'#9#9
        'Dt_Alt'#9'9'#9'Dt. Atualiz.'#9#9
        'Reposicao'#9'10'#9'Reposicão'#9'F'#9
        'VALOR'#9'10'#9'Vlr Estoque R$'#9'F'#9
        'ValorDiferenca'#9'10'#9'Valor Diferenca'#9'F'#9
        'Status'#9'11'#9'Status'#9'F'#9
        'LocalArmazem'#9'50'#9'LocalArmazem'#9'F'#9
        'ConsumoTresMeses'#9'10'#9'Consumo Ult. 3 Meses'#9'F')
      MemoAttributes = [mSizeable, mWordWrap, mDisableDialog]
      IniAttributes.FileName = 'delphi32.ini'
      IniAttributes.SectionName = 'FPcPGrid'
      IniAttributes.Delimiter = ';;'
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      EditControlOptions = [ecoCheckboxSingleClick, ecoSearchOwnerForm]
      Anchors = [akLeft, akTop, akRight, akBottom]
      Color = 14811135
      DataSource = DS_INV
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      KeyOptions = []
      Options = [dgEditing, dgAlwaysShowEditor, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
      ParentFont = False
      TabOrder = 2
      TitleAlignment = taLeftJustify
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlue
      TitleFont.Height = -11
      TitleFont.Name = 'Comic Sans MS'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = False
      OnCalcCellColors = GridCalcCellColors
      OnDblClick = GridDblClick
      OnKeyUp = GridKeyUp
      FooterColor = clInactiveCaption
      FooterCellColor = clGrayText
    end
    object Panel4: TPanel
      Left = 6
      Top = 4
      Width = 801
      Height = 175
      Anchors = [akLeft, akTop, akRight]
      BevelInner = bvLowered
      TabOrder = 0
      TabStop = True
      object Label7: TLabel
        Left = 45
        Top = 8
        Width = 70
        Height = 18
        Caption = 'Fornecedor'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label1: TLabel
        Left = 5
        Top = 36
        Width = 110
        Height = 18
        Caption = 'Grupo de Material'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label57: TLabel
        Left = 64
        Top = 63
        Width = 51
        Height = 18
        Caption = 'Material'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label3: TLabel
        Left = 19
        Top = 118
        Width = 96
        Height = 18
        Caption = 'Num. Inventário'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label2: TLabel
        Left = 89
        Top = 91
        Width = 26
        Height = 18
        Caption = 'Lote'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label5: TLabel
        Left = 216
        Top = 120
        Width = 80
        Height = 17
        Caption = '(filtro apenas)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label6: TLabel
        Left = 582
        Top = 6
        Width = 7
        Height = 18
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label8: TLabel
        Left = 582
        Top = 32
        Width = 7
        Height = 18
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label9: TLabel
        Left = 582
        Top = 60
        Width = 7
        Height = 18
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label10: TLabel
        Left = 288
        Top = 87
        Width = 7
        Height = 18
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label11: TLabel
        Left = 302
        Top = 124
        Width = 278
        Height = 15
        Caption = '*  Participam da geração porém NÃO são obrigatórios.'
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label4: TLabel
        Left = 83
        Top = 150
        Width = 32
        Height = 18
        Caption = 'Local'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object cboForn: TwwDBLookupCombo
        Left = 122
        Top = 5
        Width = 457
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'FOR_CODI'#9'15'#9'FOR_CODI'#9'F')
        LookupTable = Q_FORN
        LookupField = 'FOR_CODI'
        ParentFont = False
        TabOrder = 0
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        ShowMatchText = True
        OnCloseUp = cboFornCloseUp
        OnEnter = cboFornEnter
        OnExit = cboFornExit
        OnKeyUp = cboFornKeyUp
      end
      object cboGr: TwwDBLookupCombo
        Left = 122
        Top = 32
        Width = 457
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'GRU_DESC'#9'40'#9'GRU_DESC'#9'F')
        LookupTable = Q_GRMAT
        LookupField = 'GRU_ID'
        ParentFont = False
        TabOrder = 1
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        ShowMatchText = True
        OnCloseUp = cboFornCloseUp
        OnEnter = cboGrEnter
        OnExit = cboGrExit
        OnKeyUp = cboFornKeyUp
      end
      object cboMat: TwwDBLookupCombo
        Left = 122
        Top = 60
        Width = 457
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'MAT_DESC'#9'60'#9'MAT_DESC'#9'F')
        LookupTable = Q_MATERIAL
        LookupField = 'MAT_ID'
        ParentFont = False
        TabOrder = 2
        AutoDropDown = True
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
        ShowMatchText = True
        OnCloseUp = cboFornCloseUp
        OnEnter = cboMatEnter
        OnExit = cboMatExit
        OnKeyUp = cboFornKeyUp
      end
      object txtInv: TwwDBEdit
        Left = 122
        Top = 114
        Width = 90
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        UnboundDataType = wwDefault
        WantReturns = False
        WordWrap = False
        OnEnter = txtInvEnter
        OnExit = txtInvExit
        OnKeyUp = cboFornKeyUp
      end
      object txtLote: TwwDBEdit
        Left = 122
        Top = 87
        Width = 154
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        UnboundDataType = wwDefault
        WantReturns = False
        WordWrap = False
        OnEnter = txtInvEnter
        OnExit = txtInvExit
        OnKeyUp = cboFornKeyUp
      end
      object ckDif: TCheckBox
        Left = 301
        Top = 87
        Width = 306
        Height = 17
        Caption = 'Só os que tenham diferença (tb. é filtro para relat.)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        OnClick = ckDifClick
      end
      object ckQt: TCheckBox
        Left = 301
        Top = 106
        Width = 263
        Height = 17
        Caption = 'Só os que tenham Qtde. Inv. não preenchida'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        OnClick = ckDifClick
      end
      object Panel3: TPanel
        Left = 601
        Top = 2
        Width = 90
        Height = 52
        BevelInner = bvLowered
        TabOrder = 7
        object RadioButton1: TRadioButton
          Left = 8
          Top = 34
          Width = 59
          Height = 17
          Caption = 'Todos'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Comic Sans MS'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object rdSal: TRadioButton
          Left = 8
          Top = 11
          Width = 65
          Height = 17
          Caption = 'C/ Saldo'
          Checked = True
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Comic Sans MS'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
        end
      end
      object cboLocal: TwwDBLookupCombo
        Left = 120
        Top = 145
        Width = 456
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        DropDownAlignment = taLeftJustify
        Selected.Strings = (
          'Descricao'#9'50'#9'Descricao'#9'F')
        LookupTable = qrLocal
        LookupField = 'Local_ID'
        ParentFont = False
        TabOrder = 8
        AutoDropDown = False
        ShowButton = True
        PreciseEditRegion = False
        AllowClearKey = False
      end
      object btnGetMaterial: TBitBtn
        Left = 593
        Top = 57
        Width = 32
        Height = 28
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 9
        OnClick = btnGetMaterialClick
        Glyph.Data = {
          
          4242424242424242424242424242424242424242424242424242}
      end
      object chkConsumoMedio: TCheckBox
        Left = 592
        Top = 148
        Width = 209
        Height = 17
        Caption = 'Calcular consumo médio'
        TabOrder = 10
      end
    end
    object Panel1: TPanel
      Left = 807
      Top = 4
      Width = 152
      Height = 174
      Anchors = [akTop, akRight]
      BevelInner = bvLowered
      TabOrder = 1
      object btPesq: TSpeedButton
        Left = 7
        Top = 48
        Width = 91
        Height = 28
        Hint = 'Pesquisar'
        Caption = 'Pesquisar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          4848484848484848484848484848480A10202017484848484848}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = btPesqClick
      end
      object btLimp: TSpeedButton
        Left = 7
        Top = 76
        Width = 91
        Height = 29
        Hint = 'Limpar Filtro'
        Caption = 'Limpar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = btLimpClick
      end
      object btGera: TSpeedButton
        Left = 7
        Top = 105
        Width = 91
        Height = 28
        Hint = 'Geração de Iventário'
        Caption = 'Gerar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5A5}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = btGeraClick
      end
      object spRelat1: TSpeedButton
        Left = 7
        Top = 3
        Width = 46
        Height = 29
        Hint = 'Visualizar Relatório de Conferência'
        Caption = 'C'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          6464686E6E828282828282828282}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = spRelat1Click
      end
      object spRelat2: TSpeedButton
        Left = 53
        Top = 3
        Width = 45
        Height = 29
        Hint = 'Visualizar Relatório de Preenchimento'
        Caption = 'P'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          6464686E6E828282828282828282}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = spRelat2Click
      end
      object btRet: TSpeedButton
        Left = 55
        Top = 135
        Width = 41
        Height = 23
        Hint = 'Atualização de Almoxarifado Retroativa'
        Caption = 'R'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00E33777777777777733EEEEEEEEEEEEEEE33333333333333333}
        NumGlyphs = 2
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        Visible = False
        OnClick = btRetClick
      end
      object btAtual: TSpeedButton
        Left = 7
        Top = 133
        Width = 91
        Height = 28
        Hint = 
          'Efetua as devidas entradas e saídas de acordo com os ítens inven' +
          'tariados.'
        Caption = 'Executar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          }
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = btAtualClick
      end
      object CheckBox1: TCheckBox
        Left = 8
        Top = 32
        Width = 89
        Height = 17
        Caption = 'Ocultar campos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = CheckBox1Click
      end
      object BitBtn3: TBitBtn
        Left = 100
        Top = 3
        Width = 37
        Height = 29
        TabOrder = 1
        OnClick = BitBtn3Click
        Glyph.Data = {
          
          171717171717521B7D1D1D1D66660F0F0F191941530117171717}
      end
      object Panel5: TPanel
        Left = 94
        Top = 31
        Width = 147
        Height = 146
        TabOrder = 2
        Visible = False
        object chkLote: TCheckBox
          Left = 8
          Top = 8
          Width = 97
          Height = 17
          Caption = 'Lote'
          TabOrder = 0
        end
        object Fornecedor: TCheckBox
          Left = 8
          Top = 24
          Width = 97
          Height = 17
          Caption = 'Fornecedor'
          TabOrder = 1
        end
        object chkMarca: TCheckBox
          Left = 8
          Top = 40
          Width = 97
          Height = 17
          Caption = 'Marca'
          TabOrder = 2
        end
        object Validade: TCheckBox
          Left = 8
          Top = 56
          Width = 97
          Height = 17
          Caption = 'Validade'
          TabOrder = 3
        end
        object QtdeAlmox: TCheckBox
          Left = 8
          Top = 72
          Width = 97
          Height = 17
          Caption = 'Qtde Almox'
          TabOrder = 4
        end
        object QtdeInv: TCheckBox
          Left = 8
          Top = 88
          Width = 97
          Height = 17
          Caption = 'Qtde Inv'
          TabOrder = 5
        end
        object QtdeDif: TCheckBox
          Left = 8
          Top = 104
          Width = 97
          Height = 17
          Caption = 'Qtde Dif'
          TabOrder = 6
        end
        object btn1: TButton
          Left = 96
          Top = 112
          Width = 43
          Height = 25
          Caption = 'Ok'
          TabOrder = 7
          OnClick = btn1Click
        end
      end
    end
    object cbpTp: TwwDBComboBox
      Left = 446
      Top = 318
      Width = 187
      Height = 25
      ShowButton = True
      Style = csDropDown
      MapList = True
      AllowClearKey = False
      AutoDropDown = True
      ButtonStyle = cbsEllipsis
      DataField = 'Tipo'
      DataSource = DS_INV
      DropDownCount = 8
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ItemHeight = 0
      Items.Strings = (
        'Automático'#9'A'
        'Manual'#9'M')
      ParentFont = False
      Sorted = False
      TabOrder = 3
      UnboundDataType = wwDefault
    end
    object cboSt: TwwDBComboBox
      Left = 454
      Top = 358
      Width = 187
      Height = 25
      ShowButton = True
      Style = csDropDown
      MapList = True
      AllowClearKey = False
      AutoDropDown = True
      ButtonStyle = cbsEllipsis
      DataField = 'Tipo'
      DataSource = DS_INV
      DropDownCount = 8
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -12
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ItemHeight = 0
      Items.Strings = (
        'Aberto'#9'A'
        'Gerado'#9'G'
        'Com erro (corrigir)'#9'E')
      ParentFont = False
      Sorted = False
      TabOrder = 4
      UnboundDataType = wwDefault
    end
    object Panel6: TPanel
      Left = 2
      Top = 417
      Width = 962
      Height = 116
      Align = alBottom
      TabOrder = 5
      object Label46: TLabel
        Left = -2
        Top = 13
        Width = 31
        Height = 18
        Caption = 'Obs:.'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object SpeedButton4: TSpeedButton
        Left = -2
        Top = 34
        Width = 32
        Height = 45
        Hint = 'Salvar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton4Click
      end
      object Label13: TLabel
        Left = -2
        Top = 80
        Width = 165
        Height = 15
        Caption = '"Esc" para alterações indevidas!'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label16: TLabel
        Left = 422
        Top = 14
        Width = 122
        Height = 17
        Caption = 'Alteração do Custo M.'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label17: TLabel
        Left = 422
        Top = 45
        Width = 51
        Height = 18
        Caption = 'Custo M.'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object SpeedButton6: TSpeedButton
        Left = 553
        Top = 35
        Width = 32
        Height = 44
        Hint = 'Gravar'
        Glyph.Data = {
          
          B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton6Click
      end
      object Label12: TLabel
        Left = 432
        Top = 80
        Width = 349
        Height = 15
        Caption = 
          'Duplo clique sobre a coluna Qtde. Inv. repete valor de Almoxarif' +
          'ado'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label15: TLabel
        Left = 598
        Top = 45
        Width = 32
        Height = 18
        Caption = 'Data:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -13
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object Label14: TLabel
        Left = 601
        Top = 14
        Width = 179
        Height = 17
        Caption = 'Alteração de Data de referência'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
      end
      object bt_gravar: TSpeedButton
        Left = 752
        Top = 35
        Width = 28
        Height = 44
        Hint = 'Gravar'
        Glyph.Data = {
          
          B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
        ParentShowHint = False
        ShowHint = True
        OnClick = bt_gravarClick
      end
      object DBMemo1: TDBMemo
        Left = 32
        Top = 16
        Width = 380
        Height = 63
        DataField = 'Obs'
        DataSource = DS_INV
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        ScrollBars = ssVertical
        TabOrder = 0
      end
      object Edit1: TEdit
        Left = 478
        Top = 38
        Width = 67
        Height = 25
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnEnter = Edit1Enter
        OnExit = Edit1Exit
      end
      object Data1: TMaskEdit
        Left = 633
        Top = 40
        Width = 108
        Height = 23
        AutoSize = False
        EditMask = '!99/99/00 90:00;1;_'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        MaxLength = 14
        ParentFont = False
        TabOrder = 2
        Text = '  /  /     :  '
        OnEnter = Data1Enter
        OnExit = Data1Exit
      end
    end
  end
  object Q_FORN: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT '#9'FOR_CODI,'
      '              '#9'FOR_RAZA'
      'FROM '#9'CPAFORN'
      'WHERE F_ALMOX = '#39'S'#39
      'ORDER BY FOR_CODI')
    ValidateWithMask = True
    Left = 848
    Top = 276
    object Q_FORNFOR_CODI: TStringField
      DisplayWidth = 15
      FieldName = 'FOR_CODI'
      Origin = 'CURA.CPAFORN.FOR_CODI'
      FixedChar = True
      Size = 15
    end
    object Q_FORNFOR_RAZA: TStringField
      FieldName = 'FOR_RAZA'
      Origin = 'CURA.CPAFORN.FOR_RAZA'
      Visible = False
      FixedChar = True
      Size = 50
    end
  end
  object Q_GRMAT: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT'#9#9'GRU_ID,'
      #9#9'GRU_DESC'
      'FROM'#9#9'GRUPO_MATERIAL'
      'ORDER BY'#9'GRU_DESC')
    ValidateWithMask = True
    Left = 408
    Top = 266
    object Q_GRMATGRU_DESC: TStringField
      DisplayWidth = 40
      FieldName = 'GRU_DESC'
      Origin = 'CURA.GRUPO_MATERIAL.GRU_DESC'
      FixedChar = True
      Size = 40
    end
    object Q_GRMATGRU_ID: TAutoIncField
      FieldName = 'GRU_ID'
      Origin = 'CURA.GRUPO_MATERIAL.GRU_ID'
      Visible = False
    end
  end
  object Q_MATERIAL: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT MAT_ID,'
      '              CodigoMaterial + '#39' '#39' + MAT_DESC AS MAT_DESC'
      'FROM MATERIAIS'
      'WHERE GENERICO NOT IN ('#39'S'#39')'
      'AND MAT_ATIVO = '#39'S'#39
      'ORDER BY MAT_DESC')
    ValidateWithMask = True
    Left = 506
    Top = 317
    object Q_MATERIALMAT_DESC: TStringField
      DisplayWidth = 60
      FieldName = 'MAT_DESC'
      Origin = 'CURA.MATERIAIS.MAT_DESC'
      FixedChar = True
      Size = 60
    end
    object Q_MATERIALMAT_ID: TAutoIncField
      FieldName = 'MAT_ID'
      Origin = 'CURA.MATERIAIS.MAT_ID'
      Visible = False
    end
  end
  object Q_INV: TwwQuery
    CachedUpdates = True
    BeforePost = Q_INVBeforePost
    AfterPost = Q_INVAfterPost
    AfterScroll = Q_INVAfterScroll
    OnCalcFields = Q_INVCalcFields
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT  I.*,'
      '        MAT.MAT_DESC, MAT.LocalArmazem, '
      #9'FORN.FOR_CODI,'
      #9'MAT.UNI_ID,'
      #9'M.MARCA_DESC,'
      #9'round(I.QTDE_INV,2) * ROUND(I.REPOSICAO,2) AS VALOR'
      ', MediaCompras3Meses as ConsumoTresMeses'
      'FROM    '#9'INV I,'
      #9'MATERIAIS MAT,'
      #9'CPAFORN FORN,'
      #9'MARCAS M'
      'WHERE   I.MAT_ID = MAT.MAT_ID'
      'AND     I.FOR_CODI = FORN.FOR_CODI'
      'AND      I.NUM_INV =(SELECT MAX(NUM_INV) NUM_INV FROM INV)'
      'AND'#9'M.MARCA_ID = I.MARCA_ID'
      'ORDER BY I.NUM_INV,'
      #9' MAT.MAT_DESC')
    UpdateObject = UPD_INV
    ControlType.Strings = (
      'Tipo;CustomEdit;cbpTp'
      'Status;CustomEdit;cboSt;F')
    ValidateWithMask = True
    Left = 594
    Top = 295
    object Q_INVNum_Inv: TIntegerField
      DisplayLabel = 'Inv.'
      DisplayWidth = 6
      FieldName = 'Num_Inv'
    end
    object Q_INVFor_Codi: TStringField
      DisplayLabel = 'Fornecedor'
      DisplayWidth = 15
      FieldName = 'For_Codi'
      FixedChar = True
      Size = 15
    end
    object Q_INVMAT_DESC: TStringField
      DisplayLabel = 'Material'
      DisplayWidth = 24
      FieldName = 'MAT_DESC'
      FixedChar = True
      Size = 60
    end
    object Q_INVMARCA_DESC: TStringField
      DisplayLabel = 'Marca'
      DisplayWidth = 10
      FieldName = 'MARCA_DESC'
      FixedChar = True
    end
    object Q_INVN_Lote: TStringField
      DisplayLabel = 'Lote'
      DisplayWidth = 8
      FieldName = 'N_Lote'
      FixedChar = True
    end
    object Q_INVQtde_Almox: TFloatField
      DisplayLabel = 'Qtde. Almox.'
      DisplayWidth = 10
      FieldName = 'Qtde_Almox'
      DisplayFormat = '###,###,##0.00'
      EditFormat = '###,###,##0.00'
    end
    object Q_INVQtde_Inv: TFloatField
      DisplayLabel = 'Qtde. Inv.'
      DisplayWidth = 10
      FieldName = 'Qtde_Inv'
      DisplayFormat = '###,###,##0.00'
      EditFormat = '###,###,##0.00'
    end
    object Q_INVValid: TDateTimeField
      DisplayWidth = 10
      FieldName = 'Valid'
      DisplayFormat = 'dd/mm/yy'
      EditMask = '!99/99/00;1;_'
    end
    object Q_INVQtdeCalc: TFloatField
      DisplayLabel = 'Qtde Mov.'
      DisplayWidth = 10
      FieldKind = fkCalculated
      FieldName = 'QtdeCalc'
      DisplayFormat = '###,###,##0.00'
      EditFormat = '###,###,##0.00'
      Calculated = True
    end
    object Q_INVData: TDateTimeField
      DisplayWidth = 9
      FieldName = 'Data'
      DisplayFormat = 'dd/mm/yy'
    end
    object Q_INVTipo: TStringField
      DisplayWidth = 9
      FieldName = 'Tipo'
      FixedChar = True
      Size = 1
    end
    object Q_INVUsuario: TStringField
      DisplayLabel = 'Usuário Inv.'
      DisplayWidth = 10
      FieldName = 'Usuario'
      FixedChar = True
    end
    object Q_INVDt_Alt: TDateTimeField
      DisplayLabel = 'Dt. Atualiz.'
      DisplayWidth = 9
      FieldName = 'Dt_Alt'
      DisplayFormat = 'dd/mm/yy'
    end
    object Q_INVReposicao: TFloatField
      DisplayLabel = 'Reposicão'
      DisplayWidth = 10
      FieldName = 'Reposicao'
      DisplayFormat = '###,###,##0.00'
    end
    object Q_INVVALOR: TFloatField
      DisplayLabel = 'Vlr Estoque R$'
      DisplayWidth = 10
      FieldName = 'VALOR'
      currency = True
    end
    object Q_INVValorDiferenca: TFloatField
      DisplayLabel = 'Valor Diferenca'
      DisplayWidth = 10
      FieldKind = fkCalculated
      FieldName = 'ValorDiferenca'
      DisplayFormat = '###,###,##0.00'
      Calculated = True
    end
    object Q_INVStatus: TStringField
      DisplayWidth = 11
      FieldName = 'Status'
      FixedChar = True
      Size = 1
    end
    object Q_INVLocalArmazem: TStringField
      DisplayWidth = 50
      FieldName = 'LocalArmazem'
      FixedChar = True
      Size = 255
    end
    object Q_INVConsumoTresMeses: TIntegerField
      DisplayLabel = 'Consumo Ult. 3 Meses'
      DisplayWidth = 10
      FieldName = 'ConsumoTresMeses'
    end
    object Q_INVCusto_Medio_Inv: TFloatField
      DisplayLabel = 'Custo Médio'
      DisplayWidth = 12
      FieldName = 'Custo_Medio_Inv'
      Visible = False
      currency = True
    end
    object Q_INVInv_ID: TAutoIncField
      FieldName = 'Inv_ID'
      Visible = False
    end
    object Q_INVMat_ID: TIntegerField
      FieldName = 'Mat_ID'
      Visible = False
    end
    object Q_INVMarca_ID: TIntegerField
      FieldName = 'Marca_ID'
      Visible = False
    end
    object Q_INVLocal_ID: TIntegerField
      FieldName = 'Local_ID'
      Visible = False
    end
    object Q_INVObs: TMemoField
      FieldName = 'Obs'
      Visible = False
      BlobType = ftMemo
    end
    object Q_INVUNI_ID: TIntegerField
      FieldName = 'UNI_ID'
      Visible = False
    end
  end
  object DS_INV: TDataSource
    DataSet = Q_INV
    Left = 642
    Top = 295
  end
  object UPD_INV: TUpdateSQL
    ModifySQL.Strings = (
      'update INV'
      'set'
      '  Data = :Data,'
      '  N_Lote = :N_Lote,'
      '  Valid = :Valid,'
      '  Qtde_Inv = :Qtde_Inv,'
      '  Usuario = :Usuario,'
      '  Dt_Alt = :Dt_Alt,'
      '  Obs = :Obs,'
      '  Custo_Medio_Inv = :Custo_Medio_Inv'
      'where'
      '  Inv_ID = :OLD_Inv_ID')
    InsertSQL.Strings = (
      'insert into INV'
      
        '  (Data, N_Lote, Valid, Qtde_Inv, Usuario, Dt_Alt, Obs, Custo_Me' +
        'dio_Inv)'
      'values'
      
        '  (:Data, :N_Lote, :Valid, :Qtde_Inv, :Usuario, :Dt_Alt, :Obs, :' +
        'Custo_Medio_Inv)')
    DeleteSQL.Strings = (
      'delete from INV'
      'where'
      '  Inv_ID = :OLD_Inv_ID')
    Left = 698
    Top = 295
  end
  object SP_INV: TStoredProc
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    StoredProcName = 'dbo.SP_INV'
    Left = 104
    Top = 270
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@FOR_CODI'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@GRUPO_ID'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@MAT_ID'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@USUARIO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@N_LOTE'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@TODOS'
        ParamType = ptInput
      end>
  end
  object Q_AUX: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    ValidateWithMask = True
    Left = 352
    Top = 266
  end
  object SP_ENC_INV: TStoredProc
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    StoredProcName = 'dbo.SP_ENC_INV'
    Left = 165
    Top = 270
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftInteger
        Name = '@FORNECEDOR_ID'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@N_LOTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@MATERIAL_ID'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@NUM_INV'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@USUARIO'
        ParamType = ptInput
      end
      item
        DataType = ftFloat
        Name = '@QTDE'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@DATA'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@TIPO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@MSG'
        ParamType = ptOutput
      end>
  end
  object SP_AJALM: TwwStoredProc
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    StoredProcName = 'dbo.SP_ATUAL_LOTE'
    ValidateWithMask = True
    Left = 212
    Top = 368
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@N_LOTE'
        ParamType = ptInput
      end
      item
        DataType = ftFloat
        Name = '@QTDE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@MAT_ID'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@FOR_CODI'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@MOT_ID'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@TIPO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@USUARIO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@MSG'
        ParamType = ptOutput
      end
      item
        DataType = ftInteger
        Name = '@TD_ID'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@DOCTO'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@REC_ITEM_ID'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@DATAX'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@MARCA_ID'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@VALID'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@LOCAL_ID'
        ParamType = ptInput
      end>
  end
  object qrLocal: TADOQuery
    Connection = FMenu.ADOConnection
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM LOCAL'
      'ORDER BY DESCRICAO')
    Left = 168
    Top = 237
    object qrLocalLocal_ID: TAutoIncField
      FieldName = 'Local_ID'
      ReadOnly = True
    end
    object qrLocalDescricao: TStringField
      FieldName = 'Descricao'
      Size = 50
    end
    object qrLocalDisponivelSN: TStringField
      FieldName = 'DisponivelSN'
      FixedChar = True
      Size = 1
    end
    object qrLocalCONSUMO: TStringField
      FieldName = 'CONSUMO'
      FixedChar = True
      Size = 1
    end
    object qrLocalCONTROLA_ESTOQUE: TStringField
      FieldName = 'CONTROLA_ESTOQUE'
      FixedChar = True
      Size = 1
    end
  end
  object qrAux: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT TOP 1'
      'REPOSICAO'
      'FROM Req_Item'
      'WHERE Mat_ID = :Mat_ID'
      'AND REPOSICAO IS NOT NULL'
      'ORDER BY Req_ID DESC')
    ValidateWithMask = True
    Left = 328
    Top = 337
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'Mat_ID'
        ParamType = ptUnknown
      end>
    object qrAuxREPOSICAO: TFloatField
      FieldName = 'REPOSICAO'
    end
  end
  object qryLotes: TADOQuery
    Connection = FMenu.ADOConnection
    Parameters = <
      item
        Name = 'MAT_ID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'N_LOTE'
        DataType = ftFixedChar
        NumericScale = 255
        Precision = 255
        Size = 20
        Value = Null
      end
      item
        Name = 'FOR_CODI'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 15
        Value = Null
      end
      item
        Name = 'MARCA_ID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'LOCAL_ID'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT'#9'*'
      ''
      'FROM ALMOX'
      ''
      'WHERE MAT_ID = :MAT_ID'
      ''
      'AND N_LOTE = :N_LOTE'
      ''
      'AND FOR_CODI = :FOR_CODI'
      ''
      'AND MARCA_ID = :MARCA_ID'
      ''
      'AND LOCAL_ID = :LOCAL_ID'
      ''
      'AND TIPO = '#39'E'#39
      ''
      'AND QTDE_RETIRADA IS NULL ')
    Left = 744
    Top = 369
    object qryLotesAlmox_ID: TAutoIncField
      FieldName = 'Almox_ID'
      ReadOnly = True
    end
    object qryLotesAlmox_ID_Mestre: TIntegerField
      FieldName = 'Almox_ID_Mestre'
    end
    object qryLotesFor_Codi: TStringField
      FieldName = 'For_Codi'
      FixedChar = True
      Size = 15
    end
    object qryLotesMat_ID: TIntegerField
      FieldName = 'Mat_ID'
    end
    object qryLotesLocal_ID: TIntegerField
      FieldName = 'Local_ID'
    end
    object qryLotesMarca_ID: TIntegerField
      FieldName = 'Marca_ID'
    end
    object qryLotesN_Lote: TWideStringField
      FieldName = 'N_Lote'
    end
    object qryLotesValid: TDateTimeField
      FieldName = 'Valid'
    end
    object qryLotesQtde: TFloatField
      FieldName = 'Qtde'
    end
    object qryLotesQtde_Retirada: TFloatField
      FieldName = 'Qtde_Retirada'
    end
    object qryLotesQtde_Input: TFloatField
      FieldName = 'Qtde_Input'
    end
    object qryLotesTipo: TStringField
      FieldName = 'Tipo'
      FixedChar = True
      Size = 1
    end
    object qryLotesMot_ID: TIntegerField
      FieldName = 'Mot_ID'
    end
    object qryLotesUni_ID: TIntegerField
      FieldName = 'Uni_ID'
    end
    object qryLotesUni_Sigla: TStringField
      FieldName = 'Uni_Sigla'
      Size = 2
    end
    object qryLotesObs: TMemoField
      FieldName = 'Obs'
      BlobType = ftMemo
    end
    object qryLotesData: TDateTimeField
      FieldName = 'Data'
    end
    object qryLotesUsuario: TWideStringField
      FieldName = 'Usuario'
      Size = 50
    end
    object qryLotesTD_ID: TIntegerField
      FieldName = 'TD_ID'
    end
    object qryLotesDocto: TStringField
      FieldName = 'Docto'
    end
    object qryLotesRec_Item_ID: TIntegerField
      FieldName = 'Rec_Item_ID'
    end
    object qryLotesCusto: TFloatField
      FieldName = 'Custo'
    end
    object qryLotesREPOSICAO: TFloatField
      FieldName = 'REPOSICAO'
    end
  end
  object dsExcel: TDataSource
    DataSet = Q_INV
    Left = 264
    Top = 273
  end
end
