object frmPesqMateriais: TfrmPesqMateriais
  Left = 129
  Top = 35
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Pesquisa de Materiais'
  ClientHeight = 524
  ClientWidth = 815
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Verdana'
  Font.Style = [fsBold]
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 815
    Height = 181
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 15
      Top = 4
      Width = 59
      Height = 16
      Caption = 'Material'
    end
    object Label2: TLabel
      Left = 15
      Top = 108
      Width = 43
      Height = 16
      Caption = 'Grupo'
    end
    object Label3: TLabel
      Left = 327
      Top = 108
      Width = 44
      Height = 16
      Caption = 'Marca'
    end
    object Label4: TLabel
      Left = 15
      Top = 56
      Width = 124
      Height = 16
      Caption = 'Parte do Material'
    end
    object Label5: TLabel
      Left = 327
      Top = 56
      Width = 135
      Height = 16
      Caption = 'Código do Material'
    end
    object cboMaterial: TwwDBLookupCombo
      Left = 12
      Top = 24
      Width = 621
      Height = 24
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'MAT_DESC'#9'60'#9'MAT_DESC'#9'F')
      LookupTable = qrMateriais
      LookupField = 'MAT_ID'
      TabOrder = 0
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = False
      OnEnter = cboMaterialEnter
    end
    object BitBtn1: TBitBtn
      Left = 671
      Top = 1
      Width = 140
      Height = 28
      Caption = 'Pesquisar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        4541588080808080808080803C3F453A53358080808080808080}
    end
    object cboGrupo: TwwDBLookupCombo
      Left = 12
      Top = 128
      Width = 301
      Height = 24
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'GRU_DESC'#9'40'#9'GRU_DESC'#9'F')
      LookupTable = qrGrupo
      LookupField = 'GRU_ID'
      TabOrder = 2
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = False
    end
    object cboMarca: TwwDBLookupCombo
      Left = 324
      Top = 128
      Width = 309
      Height = 24
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'MARCA_DESC'#9'20'#9'MARCA_DESC'#9'F')
      LookupTable = qrMarcas
      LookupField = 'MARCA_ID'
      TabOrder = 3
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = False
    end
    object BitBtn2: TBitBtn
      Left = 671
      Top = 30
      Width = 140
      Height = 28
      Caption = 'Limpar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
    object BitBtn3: TBitBtn
      Left = 671
      Top = 59
      Width = 140
      Height = 28
      Caption = 'Adicionar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
    object BitBtn4: TBitBtn
      Left = 671
      Top = 88
      Width = 140
      Height = 28
      Caption = 'Visualizar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        4242424242424242424242424242424242424242424242424242}
    end
    object edtMaterial: TEdit
      Left = 12
      Top = 76
      Width = 297
      Height = 24
      TabOrder = 7
      OnEnter = edtMaterialEnter
    end
    object BitBtn5: TBitBtn
      Left = 671
      Top = 116
      Width = 140
      Height = 28
      Caption = 'Imprimir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        4242424242424242424242424242424242424242424242424242}
    end
    object BitBtn6: TBitBtn
      Left = 671
      Top = 144
      Width = 140
      Height = 28
      Caption = 'Codigos XML'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        4242424242424242424242424242424242424242424242424242}
    end
    object edtCodigo: TEdit
      Left = 324
      Top = 76
      Width = 297
      Height = 24
      TabOrder = 10
      OnEnter = edtMaterialEnter
    end
  end
  object wwDBGrid1: TwwDBGrid
    Left = 0
    Top = 181
    Width = 815
    Height = 343
    Selected.Strings = (
      'CodigoMaterial'#9'10'#9'Código'#9'F'
      'MAT_DESC'#9'40'#9'MATERIAL'
      'GRU_DESC'#9'30'#9'GRUPO'
      'MARCA_DESC'#9'15'#9'MARCA')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    Align = alClient
    DataSource = dtDBGrid
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Verdana'
    Font.Style = []
    Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentFont = False
    ReadOnly = True
    TabOrder = 1
    TitleAlignment = taLeftJustify
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Verdana'
    TitleFont.Style = [fsBold]
    TitleLines = 1
    TitleButtons = False
    OnDblClick = BitBtn4Click
  end
  object qrMateriais: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT  MAT_ID, MAT_DESC  FROM MATERIAIS'
      'ORDER BY MAT_DESC')
    Left = 552
    Top = 216
    object qrMateriaisMAT_DESC: TStringField
      DisplayWidth = 60
      FieldName = 'MAT_DESC'
      Size = 60
    end
    object qrMateriaisMAT_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'MAT_ID'
      ReadOnly = True
      Visible = False
    end
  end
  object qrGrupo: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT GRU_ID, GRU_DESC FROM GRUPO_MATERIAL'
      'ORDER BY GRU_DESC')
    Left = 552
    Top = 272
    object qrGrupoGRU_DESC: TStringField
      DisplayWidth = 40
      FieldName = 'GRU_DESC'
      Size = 40
    end
    object qrGrupoGRU_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'GRU_ID'
      ReadOnly = True
      Visible = False
    end
  end
  object qrMarcas: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT MARCA_ID, MARCA_DESC FROM MARCAS'
      'ORDER BY MARCA_DESC')
    Left = 552
    Top = 328
    object qrMarcasMARCA_DESC: TStringField
      DisplayWidth = 20
      FieldName = 'MARCA_DESC'
    end
    object qrMarcasMARCA_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'MARCA_ID'
      ReadOnly = True
      Visible = False
    end
  end
  object qrDBGrid: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT     CodigoMaterial,'
      '              MATERIAIS.MAT_ID,'
      #9'MATERIAIS.MAT_DESC, '
      #9'GRUPO_MATERIAL.GRU_DESC, '
      #9'MARCAS.MARCA_DESC'
      'FROM         '
      #9'MATERIAIS INNER JOIN'
      
        #9'GRUPO_MATERIAL ON MATERIAIS.GRU_ID = GRUPO_MATERIAL.GRU_ID INNE' +
        'R JOIN'
      
        #9'MATERIAL_MARCA ON MATERIAIS.MAT_ID = MATERIAL_MARCA.MAT_ID INNE' +
        'R JOIN'
      #9'MARCAS ON MATERIAL_MARCA.MARCA_ID = MARCAS.MARCA_ID')
    Left = 552
    Top = 384
    object qrDBGridCodigoMaterial: TStringField
      DisplayLabel = 'Código'
      DisplayWidth = 10
      FieldName = 'CodigoMaterial'
      Size = 50
    end
    object qrDBGridMAT_DESC: TStringField
      DisplayLabel = 'MATERIAL'
      DisplayWidth = 40
      FieldName = 'MAT_DESC'
      Size = 60
    end
    object qrDBGridGRU_DESC: TStringField
      DisplayLabel = 'GRUPO'
      DisplayWidth = 30
      FieldName = 'GRU_DESC'
      Size = 40
    end
    object qrDBGridMARCA_DESC: TStringField
      DisplayLabel = 'MARCA'
      DisplayWidth = 15
      FieldName = 'MARCA_DESC'
    end
    object qrDBGridMAT_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'MAT_ID'
      ReadOnly = True
      Visible = False
    end
  end
  object dtDBGrid: TDataSource
    DataSet = qrDBGrid
    Left = 616
    Top = 384
  end
  object qrCodigosXML: TADOQuery
    Connection = FMenu.ADOConnection
    Parameters = <>
    SQL.Strings = (
      'SELECT *'
      'FROM ('
      'SELECT MAT_DESC, CodigoFornecedor, '#39#39' Fornecedor, '#39#39' EAN '
      'FROM MATERIAIS '
      
        'WHERE CodigoFornecedor IS NOT NULL AND LTRIM(RTRIM(CodigoFornece' +
        'dor)) <> '#39#39
      ''
      'UNION ALL'
      ''
      'SELECT MAT_DESC, Codigo, FOR_CODI, x.EAN '
      'FROM MATERIAIS M '
      'INNER JOIN MateriaisCodigoXML X ON M.MAT_ID = X.MAT_ID'
      ')AS A'
      'ORDER BY 2')
    Left = 264
    Top = 216
    object qrCodigosXMLMAT_DESC: TStringField
      FieldName = 'MAT_DESC'
      ReadOnly = True
      Size = 100
    end
    object qrCodigosXMLCodigoFornecedor: TStringField
      FieldName = 'CodigoFornecedor'
      ReadOnly = True
      Size = 255
    end
    object qrCodigosXMLFornecedor: TStringField
      FieldName = 'Fornecedor'
      ReadOnly = True
      Size = 50
    end
    object qrCodigosXMLEAN: TStringField
      FieldName = 'EAN'
      ReadOnly = True
      Size = 50
    end
  end
end
