...
type
  TFlstVencimentosint = class(TForm)
    Q_SINT: TwwQuery;
    ...
    QRExpr3: TQRExpr;
    QRExpr1: TQRExpr;
    Q_SINTVencimento: TDateTimeField;
    QRMemo1: TQRMemo;
...
var
  FlstVencimentosint: TFlstVencimentosint;
...
procedure TFlstVencimentosint.FormCreate(Sender: TObject);
var StrTipo : String ;
    StrAnd  : String;
begin
   fTotal := 0 ;
  ...
  QRLabel4.caption := 'Relatório Sintetico de Contas a Pagar - Vencimento ' + StrTipo ;
  Q_sint.close ;
  Q_sint.SQL.clear ;
  Q_Sint.SQL.ADD('Select Vencimento, sum(ValorLiquido) as XValor from CPAPAGA A' ) ;
  StrAnd := ' where ';
  ...
  Q_Sint.SQL.ADD('group by A.Vencimento');
  Q_Sint.SQL.ADD( ' order by A.Vencimento' )  ;
  Q_Sint.open;
  QRMemo1.lines.add(StrFiltro) ;
end;

procedure TFlstVencimentosint.FormClose(Sender: TObject;
...
procedure TFlstVencimentosint.LST_VENCBeforePrint(Sender:TCustomQuickRep;
  var PrintReport: Boolean);
Var
     vLogo : TBlobField;
begin
...
procedure TFlstVencimentosint.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Q_IPAG.close ;
  Q_IPAG.SQL.clear ;
  Q_IPAG.SQL.ADD('Select sum(B.ValorPago) as YValor from CPAPAGA A ,CPAIPAG B ') ;
  Q_IPAG.SQL.ADD('where A.Numero = B.Numero and  A.Vencimento = '+ ''''+ formatdatetime('MM/DD/YYYY', Q_SintVencimento.asdatetime) +'''' ) ;
...
procedure TFlstVencimentosint.QRDBText4Print(sender: TObject;
  var Value: String);
begin
...
procedure TFlstVencimentosint.QRExpr1Print(sender: TObject;
  var Value: String);
begin
