...
uses UMenu, URELCOTACAO, UMapaCotacao, PesqOC, UOrdCom, ufrmSelEmpresa,
  ufrmBionexoUpload, UfrmBuscaMaterial;
...
begin
  ...
  {with qrAux do begin
      close;
      SQL.Clear;
      SQL.Add(' select ValorContrato from Materiais where mat_id = ' + qrItensMAT_ID.AsString);
      Open;
  end;

  if (qrAux.FieldByName('ValorContrato').AsString <> '') AND (qrAux.FieldByName('ValorContrato').AsFloat > 0) THEN begin
      qrItensUlCompra.Value := qrAux.FieldByname('ValorContrato').AsFloat;
  end
  else begin 
      with qrAux do begin
          CLOSE;
          sql.Clear;
          SQL.Add(' select [dbo].[fnGetValorReposicao](' + qrItensMAT_ID.AsString  + ', NULL, GETDATE()) as preco_unit ');
          Open;
      end;
