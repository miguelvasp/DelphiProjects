...
type
  TFRecebPesq = class(TForm)
    ...
    qrListagemRec_ID: TAutoIncField;
    qrListagemNFIS: TIntegerField;
    qrListagemSerie: TStringField;
    qrListagemDt_Entrada: TDateTimeField;
    qrListagemCFO: TStringField;
    qrListagemCfa_Sequ: TIntegerField;
    qrListagemDt_Emiss: TDateTimeField;
    qrListagemCond_Pgto_ID: TIntegerField;
    qrListagemLocal_ID: TIntegerField;
    qrListagemICMS: TFloatField;
    qrListagemIPI: TFloatField;
    qrListagemCodICMS: TStringField;
    qrListagemCodIPI: TStringField;
    qrListagemVl_ICMS: TFloatField;
    qrListagemVl_IPI: TFloatField;
    qrListagemBase_ICMS: TFloatField;
    qrListagemBase_IPI: TFloatField;
    qrListagemFrete: TStringField;
    qrListagemVl_Frete: TFloatField;
    qrListagemVl_Seguro: TFloatField;
    qrListagemVl_OutrasDesp: TFloatField;
    qrListagemVl_TotMer: TFloatField;
    qrListagemVl_TotNF: TFloatField;
    qrListagemStatus: TStringField;
    qrListagemFor_Codi: TStringField;
    qrListagemFIL_CODI: TStringField;
    qrListagemCONFERE_QTDE: TStringField;
    qrListagemCONFERE_VALIDADE: TStringField;
    qrListagemCONFERE_LOTE: TStringField;
    qrListagemCONFERE_HORARIO: TStringField;
    qrListagemCONFERE_TEMPERATURA: TStringField;
    qrListagemCONFERE_CONDICAO: TStringField;
    qrListagemCONFERE_NUMERO_LOTE: TStringField;
    qrListagemCONFERE_REGISTRO: TStringField;
    qrListagemCONFERE_OBS: TStringField;
    qrListagemCONFERE_GRAUS: TFloatField;
    qrListagemCONFERE_NC: TStringField;
    qrListagemCONFERE_OCORRENCIA: TStringField;
    qrListagemCONFERE_CONSEQUENCIA: TStringField;
    qrListagemCONFERE_ACAO: TStringField;
    qrListagemCONFERE_RESPONSAVEL: TStringField;
    qrListagemVenc: TStringField;
    btnGetMaterial: TBitBtn;
    btnApagarNotaFiscal: TBitBtn;
    qrApagarNF: TADOQuery;
    ...
    procedure btnApagarNotaFiscalClick(Sender: TObject);
    procedure Pesquisar;
...
uses UReceb, UMateriais, URelRecMerc, uDvSel, UMenu,
  ufrmRecebimentoConferencia, UfrmRecebPesqListagem, UfrmBuscaMaterial;
...
begin
   for IK := 0 TO PRED(ComponentCount) do begin
       ...
       btnApagarNotaFiscal.Visible := FMenu.sqlUsuariosExcluirNF.AsString = 'S';
end;
...
procedure TFRecebPesq.btnConfClick(Sender: TObject);
begin
    frmRecebimentoConferencia := tfrmRecebimentoConferencia.create(Self);
    frmRecebimentoConferencia.lbRecebimento.Caption := Q_RECRec_ID.AsString;
    frmRecebimentoConferencia.ShowModal;     
end;
...
begin
         ...
         FReceb.Q_REC.ParamS[0].AsInteger := StrToInt('0');
         FReceb.ShowModal;
         FReceb.Release;
...
var sql : string;
begin
    SQL := SQL + ' SELECT distinct REC.*, ';
    SQL := SQL + ' convert( varchar, (';
    SQL := SQL + ' SELECT DISTINCT Venc = STUFF(( SELECT '' '' + CONVERT(VARCHAR,Dt_Vcto, 103 ) FROM Rec_Vcto ';
    SQL := SQL + ' WHERE REC_ID = REC.Rec_ID ';
    SQL := SQL + ' FOR XML PATH ('''')), 1, 1, '''') ';
    SQL := SQL + ' FROM Rec_Vcto ';
    SQL := SQL + ' WHERE REC_ID = REC.Rec_ID';
    SQL := SQL + ' )) AS Venc';
    SQL := SQL + ' FROM Rec_NF REC  INNER JOIN Rec_Item ITEM ON REC.Rec_ID = ITEM.Rec_ID';
    ...
    SQL := SQL + ' WHERE 1 = 1 ';
          ...
          DATAE:= formatdatetime('YYYYMMDD',strtodate(MaskEdit3.text )) ;
          DATAEF:= formatdatetime('YYYYMMDD',strtodate(MaskEdit4.text )) ;
          sql := SQL +' AND CONVERT(VARCHAR, REC.DT_ENTRADA, 112) >= ' + '''' + DATAE + '''';
          sql := SQL +' AND CONVERT(VARCHAR, REC.DT_ENTRADA, 112) <= ' + '''' + DATAEF + '''';
    ...
    qrListagem.Close;
    qrListagem.SQL.Clear;
    qrListagem.SQL.add(sql);
    qrListagem.Open;

    frmRecebPesqListagem  := TfrmRecebPesqListagem.create(self);
    frmRecebPesqListagem.QuickRep1.Preview;

end;

procedure TFRecebPesq.btnGetMaterialClick(Sender: TObject);
begin
...
procedure TFRecebPesq.btnApagarNotaFiscalClick(Sender: TObject);
begin
  if Q_RECRec_ID.AsString = '' then begin
      ShowMessage('Nenhuma nota selecionada');
      Abort;
  end;

  if MessageBox(Self.Handle,  'Tem certeza que deseja excluir a nota fiscal, isto irá apagar todos os dados de estoque, custo e a propria nota fiscal, deseja continuar', 'Confirmação', MB_YESNO) = 6 then begin
      with qrApagarNF do begin
          Close;
          sql.Clear;
          SQL.Add('EXEC STP_RecebimentoDesfazerEntrada ' + Q_RECRec_ID.AsString);
          ExecSQL;

      end;
      Pesquisar;
      ShowMessage('É importante comunicar o financeiro da exclusão da nota fiscal!');
  end;
end;

procedure TFRecebPesq.Pesquisar;
Var Sql: string;
begin
     SQL_MERC:='';

     SQL:= ' SELECT 	 FORN.FOR_CODI, ' +
           '             CASE WHEN ITEM.GENERICO = ''S'' THEN ITEM.MAT_DESC ELSE MAT.CodigoMaterial + '' '' + MAT.MAT_DESC  END AS MAT_DESC , ' +
           '             REC.*,         ' +
           '             ITEM.*,       ' +
           '             ITEM.STATUS ST_ITEM ' +
