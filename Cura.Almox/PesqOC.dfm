object FPesqOrdComp: TFPesqOrdComp
  Left = 274
  Top = 87
  Width = 1036
  Height = 627
  BorderIcons = [biSystemMenu, biMinimize, biHelp]
  Caption = 'Pesquisa de O.C. - Sistema de Almoxarifado'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1020
    Height = 164
    Align = alTop
    BevelInner = bvLowered
    TabOrder = 0
    object Label1: TLabel
      Left = 10
      Top = 48
      Width = 70
      Height = 18
      Caption = 'Fornecedor'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 8
      Top = 90
      Width = 51
      Height = 18
      Caption = 'Material'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label5: TLabel
      Left = 444
      Top = 47
      Width = 39
      Height = 18
      Caption = 'Status'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 10
      Top = 6
      Width = 24
      Height = 18
      Caption = 'O.C.'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 115
      Top = 4
      Width = 47
      Height = 18
      Caption = 'Entrega'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label15: TLabel
      Left = 210
      Top = 26
      Width = 19
      Height = 18
      Caption = 'até'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 444
      Top = 91
      Width = 38
      Height = 18
      Caption = 'Marca'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label12: TLabel
      Left = 10
      Top = 139
      Width = 155
      Height = 18
      Caption = 'Duplo Clique sobre o Grid'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label8: TLabel
      Left = 180
      Top = 139
      Width = 57
      Height = 18
      Caption = '->>Aberta'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label9: TLabel
      Left = 315
      Top = 139
      Width = 42
      Height = 18
      Caption = 'Parcial'
      Color = 12572114
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label10: TLabel
      Left = 364
      Top = 139
      Width = 63
      Height = 18
      Caption = 'Finalizada'
      Color = 12572114
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label11: TLabel
      Left = 245
      Top = 139
      Width = 62
      Height = 18
      Caption = 'Cancelada'
      Font.Charset = ANSI_CHARSET
      Font.Color = clRed
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 358
      Top = 3
      Width = 50
      Height = 18
      Caption = 'Empresa'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
    end
    object Label13: TLabel
      Left = 436
      Top = 138
      Width = 146
      Height = 18
      Alignment = taCenter
      Caption = 'Emergencia ou Urgencia'
      Color = clRed
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object cboForn: TwwDBLookupCombo
      Left = 10
      Top = 65
      Width = 423
      Height = 26
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'FOR_RAZA'#9'50'#9'FOR_RAZA'#9'F')
      LookupTable = Q_FORN
      LookupField = 'FOR_CODI'
      ParentFont = False
      TabOrder = 3
      AutoDropDown = True
      ShowButton = True
      SeqSearchOptions = [ssoEnabled, ssoCaseSensitive]
      PreciseEditRegion = False
      AllowClearKey = False
      OnChange = cboFornChange
      OnEnter = cboFornEnter
      OnExit = cboFornExit
      OnKeyUp = cboFornKeyUp
    end
    object cboMat: TwwDBLookupCombo
      Left = 10
      Top = 109
      Width = 383
      Height = 26
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'MAT_DESC'#9'60'#9'MAT_DESC'#9'F')
      LookupTable = Q_MAT
      LookupField = 'MAT_ID'
      ParentFont = False
      TabOrder = 5
      AutoDropDown = True
      ShowButton = True
      SeqSearchOptions = [ssoEnabled, ssoCaseSensitive]
      PreciseEditRegion = False
      AllowClearKey = False
      OnChange = cboMatChange
      OnEnter = cboFornEnter
      OnExit = cboMatExit
      OnKeyUp = cboMatKeyUp
    end
    object txtOC: TwwDBEdit
      Left = 9
      Top = 22
      Width = 88
      Height = 26
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      UnboundDataType = wwDefault
      WantReturns = False
      WordWrap = False
      OnChange = txtOCChange
      OnEnter = txtOCEnter
      OnExit = txtOCExit
      OnKeyUp = txtOCKeyUp
    end
    object DBE_EST_ENT1: TMaskEdit
      Left = 112
      Top = 22
      Width = 87
      Height = 26
      EditMask = '99/99/9999'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      MaxLength = 10
      ParentFont = False
      TabOrder = 1
      Text = '  /  /    '
      OnEnter = DBE_EST_ENT1Enter
      OnExit = DBE_EST_ENT1Exit
    end
    object DBE_EST_ENT2: TMaskEdit
      Left = 241
      Top = 21
      Width = 87
      Height = 26
      EditMask = '99/99/9999'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      MaxLength = 10
      ParentFont = False
      TabOrder = 2
      Text = '  /  /    '
      OnChange = DBE_EST_ENT2Change
      OnEnter = DBE_EST_ENT2Enter
      OnExit = DBE_EST_ENT2Exit
      OnKeyUp = DBE_EST_ENT2KeyUp
    end
    object cboStat: TwwDBComboBox
      Left = 443
      Top = 64
      Width = 406
      Height = 26
      ShowButton = True
      Style = csDropDown
      MapList = True
      AllowClearKey = False
      AutoDropDown = True
      ButtonStyle = cbsEllipsis
      Color = clWhite
      DropDownCount = 8
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ItemHeight = 0
      Items.Strings = (
        'Cancelada'#9'C'
        'Em Aberto'#9'A'
        'Parcialmente Atendida'#9'P'
        'Totalmente Atendida'#9'T')
      ItemIndex = 1
      ParentFont = False
      Sorted = False
      TabOrder = 4
      UnboundDataType = wwDefault
      OnChange = cboStatChange
      OnEnter = cboStatEnter
      OnExit = cboStatExit
      OnKeyUp = cboStatKeyUp
    end
    object wwDBLookupCombo4: TwwDBLookupCombo
      Left = 443
      Top = 109
      Width = 406
      Height = 26
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'MARCA_DESC'#9'20'#9'MARCA_DESC'#9'F')
      LookupTable = Q_MARC
      LookupField = 'MARCA_ID'
      ParentFont = False
      TabOrder = 6
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = True
      OnChange = wwDBLookupCombo4Change
      OnEnter = wwDBLookupCombo4Enter
      OnExit = wwDBLookupCombo4Exit
      OnKeyUp = wwDBLookupCombo4KeyUp
    end
    object Panel4: TPanel
      Left = 864
      Top = 2
      Width = 169
      Height = 157
      BevelInner = bvLowered
      TabOrder = 7
      TabStop = True
      object SBT_PESQ: TSpeedButton
        Left = 7
        Top = 64
        Width = 157
        Height = 25
        Hint = 'Executa as pesquisas de acordo com os filtros.'
        Caption = '&Pesquisa'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          4242424242424242424242424242424242424242424242424242}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = SBT_PESQClick
      end
      object SBT_SELE: TSpeedButton
        Left = 7
        Top = 34
        Width = 157
        Height = 25
        Hint = 'Seleciona o fornecedor.'
        Caption = '&Limpa'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = SBT_SELEClick
      end
      object btIncluir: TSpeedButton
        Left = 7
        Top = 4
        Width = 157
        Height = 25
        Hint = 'Troca os status dos pedidos selecionados.'
        Caption = 'Incluir'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = btIncluirClick
      end
      object SpeedButton1: TSpeedButton
        Left = 7
        Top = 95
        Width = 157
        Height = 25
        Hint = 'Busca Dinâmica'
        Caption = 'Finalizar O.C'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton1Click
      end
      object SpeedButton6: TSpeedButton
        Left = 7
        Top = 126
        Width = 157
        Height = 25
        Hint = 'Cancelar Ordem de Compra e seus itens(Vistadas).'
        Caption = 'Cancelar O.C'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Comic Sans MS'
        Font.Style = []
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton6Click
      end
    end
    object wwDBLookupCombo1: TwwDBLookupCombo
      Left = 358
      Top = 20
      Width = 491
      Height = 26
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'FIL_DESC'#9'20'#9'FIL_DESC'#9'F')
      LookupTable = ADOQuery1
      LookupField = 'FIL_CODI'
      Style = csDropDownList
      ParentFont = False
      TabOrder = 8
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = False
    end
    object btnGetMaterial: TBitBtn
      Left = 394
      Top = 107
      Width = 32
      Height = 28
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      OnClick = btnGetMaterialClick
      Glyph.Data = {
        
        4242424242424242424242424242424242424242424242424242}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 164
    Width = 1020
    Height = 425
    Align = alClient
    BevelInner = bvLowered
    TabOrder = 1
    object DBG_OC: TwwDBGrid
      Left = 2
      Top = 2
      Width = 1016
      Height = 421
      Hint = 'Visualização dos Registros'
      Selected.Strings = (
        'ORD_ID'#9'7'#9'O.C.'#9#9
        'DT_ENTREGA'#9'10'#9'Entrega'#9#9
        'MAT_DESC'#9'20'#9'Material'#9#9
        'fornecedor'#9'21'#9'Fornecedor'#9#9
        'MARCA_DESC'#9'15'#9'Marca'#9#9
        'QTDE_COMP'#9'7'#9'Qtde.'#9#9
        'QTDE_COMP_REC'#9'8'#9'Qtde. Rec.'#9#9
        'MATMARCA_EMBALAGEM'#9'16'#9'Embalagem'#9#9
        'ST_ITEM'#9'15'#9'Status'#9#9
        'Autorizacao'#9'12'#9'Autorização'#9#9)
      IniAttributes.Delimiter = ';;'
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      Align = alClient
      Color = clWhite
      DataSource = DS_OC
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      KeyOptions = []
      Options = [dgEditing, dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
      ParentFont = False
      ParentShowHint = False
      ReadOnly = True
      ShowHint = True
      TabOrder = 0
      TitleAlignment = taLeftJustify
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlue
      TitleFont.Height = -11
      TitleFont.Name = 'Comic Sans MS'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = False
      OnCalcCellColors = DBG_OCCalcCellColors
      OnDblClick = DBG_OCDblClick
    end
    object cboStPesq: TwwDBComboBox
      Left = 260
      Top = 146
      Width = 192
      Height = 26
      ShowButton = True
      Style = csDropDown
      MapList = True
      AllowClearKey = False
      AutoDropDown = True
      ButtonStyle = cbsEllipsis
      Color = clWhite
      DropDownCount = 8
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Comic Sans MS'
      Font.Style = []
      ItemHeight = 0
      Items.Strings = (
        'Parcialmente Atendida'#9'P'
        'Totalmente Atendida'#9'T'
        'Em Aberto'#9'A'
        'Cancelada'#9'C')
      ParentFont = False
      Sorted = False
      TabOrder = 1
      UnboundDataType = wwDefault
    end
  end
  object Q_OC: TwwQuery
    CachedUpdates = True
    AfterPost = Q_OCAfterPost
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT '#9'top 200'#9'A.ORD_ID,'
      #9#9'A.DT_CADASTRO,'
      #9#9'A.USUARIO,'
      #9#9'A.CH_AUT,'
      #9#9'A.CH_CANCEL,'
      #9#9'A.FOR_CODI,'
      #9#9'A.NUM_OCOM,'
      #9#9'A.COND_PGTO_ID,'
      #9#9'A.OBS,'
      #9#9'A.DT_EMISSAO,'
      #9#9'A.DT_ENTREGA,'
      #9#9'A.STATUS,'
      #9#9'A.VALOR,'
      #9#9'A.DT_CANCEL,'
      #9#9'A.VL_IPI,'
      #9#9'A.VL_TOT,'
      '                                    A.TIPOCOMPRA,'
      '        '#9#9'C.MAT_ID,'
      #9#9'D.MARCA_DESC,'
      
        #9#9'CASE WHEN C.MAT_DESC IS not null THEN B.MAT_DESC ELSE C.MAT_DE' +
        'SC END AS MAT_DESC,'
      #9#9'B.STATUS ST_ITEM,'
      #9#9'B.UNI_COMP_ID,'
      #9#9'B.QTDE_COMP,'
      #9#9'B.QTDE_COMP_REC ,'
      #9#9'MM.MATMARCA_EMBALAGEM,'
      #9#9'B.ALTERAR,'
      #9#9'B.IORD_ID,'
      
        'CASE WHEN ((a.STATUS = '#39'A'#39' OR a.STATUS = '#39'P'#39') AND DATEADD(DAY, 1' +
        '0, DT_ENTREGA) <= GETDATE()  AND AutorizaOrdemVencida IS NULL) T' +
        'HEN '#39'Autorizar'#39' End as Autorizacao'
      'FROM    '#9#9'ORDEM_COMPRA A,'
      #9#9'ORDEM_COMPRAITEM B,'
      #9#9'MATERIAIS C,'
      #9#9'MARCAS D,'
      #9#9'MATERIAL_MARCA MM'
      'WHERE   '#9'A.ORD_ID = B.ORD_ID'
      'AND     '#9#9'B.MAT_ID = C.MAT_ID'
      'AND     '#9#9'B.MARCA_ID = D.MARCA_ID'
      'AND'#9#9'MM.MATMARCA_ID = B.MATMARCA_ID'
      'ORDER BY '#9'A.ORD_ID DESC')
    UpdateObject = UPD_OC
    ControlType.Strings = (
      'STATUS;CustomEdit;wwDBComboBox1'
      'ST_ITEM;CustomEdit;cboStPesq;F'
      'ALTERAR;CheckBox;S;')
    ValidateWithMask = True
    Left = 148
    Top = 312
    object Q_OCORD_ID: TAutoIncField
      DisplayLabel = 'O.C.'
      DisplayWidth = 7
      FieldName = 'ORD_ID'
      Origin = 'CURA.ORDEM_COMPRA.ORD_ID'
    end
    object Q_OCDT_ENTREGA: TDateTimeField
      DisplayLabel = 'Entrega'
      DisplayWidth = 10
      FieldName = 'DT_ENTREGA'
      Origin = 'CURA.ORDEM_COMPRA.DT_ENTREGA'
    end
    object Q_OCMAT_DESC: TStringField
      DisplayLabel = 'Material'
      DisplayWidth = 20
      FieldName = 'MAT_DESC'
      Origin = 'CURA.MATERIAIS.MAT_DESC'
      FixedChar = True
      Size = 60
    end
    object Q_OCfornecedor: TStringField
      DisplayLabel = 'Fornecedor'
      DisplayWidth = 21
      FieldKind = fkLookup
      FieldName = 'fornecedor'
      LookupDataSet = Q_FORN
      LookupKeyFields = 'FOR_CODI'
      LookupResultField = 'FOR_RAZA'
      KeyFields = 'FOR_CODI'
      Size = 28
      Lookup = True
    end
    object Q_OCMARCA_DESC: TStringField
      DisplayLabel = 'Marca'
      DisplayWidth = 15
      FieldName = 'MARCA_DESC'
      Origin = 'CURA.MARCAS.MARCA_DESC'
      FixedChar = True
    end
    object Q_OCQTDE_COMP: TFloatField
      DisplayLabel = 'Qtde.'
      DisplayWidth = 7
      FieldName = 'QTDE_COMP'
    end
    object Q_OCQTDE_COMP_REC: TFloatField
      DisplayLabel = 'Qtde. Rec.'
      DisplayWidth = 8
      FieldName = 'QTDE_COMP_REC'
    end
    object Q_OCMATMARCA_EMBALAGEM: TStringField
      DisplayLabel = 'Embalagem'
      DisplayWidth = 16
      FieldName = 'MATMARCA_EMBALAGEM'
      FixedChar = True
      Size = 15
    end
    object Q_OCST_ITEM: TStringField
      DisplayLabel = 'Status'
      DisplayWidth = 15
      FieldName = 'ST_ITEM'
      FixedChar = True
      Size = 1
    end
    object Q_OCAutorizacao: TStringField
      DisplayLabel = 'Autorização'
      DisplayWidth = 12
      FieldName = 'Autorizacao'
      FixedChar = True
      Size = 9
    end
    object Q_OCmaterial: TStringField
      DisplayLabel = 'Material'
      DisplayWidth = 20
      FieldKind = fkLookup
      FieldName = 'material'
      LookupDataSet = Q_MAT
      LookupKeyFields = 'MAT_ID'
      LookupResultField = 'MAT_DESC'
      KeyFields = 'MAT_ID'
      Visible = False
      Size = 30
      Lookup = True
    end
    object Q_OCALTERAR: TStringField
      DisplayLabel = '**'
      DisplayWidth = 1
      FieldName = 'ALTERAR'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      OnChange = Q_OCALTERARChange
      FixedChar = True
      Size = 1
    end
    object Q_OCSTATUS: TStringField
      DisplayLabel = 'Status'
      DisplayWidth = 18
      FieldName = 'STATUS'
      Origin = 'CURA.ORDEM_COMPRA.STATUS'
      Visible = False
      OnGetText = Q_OCSTATUSGetText
      FixedChar = True
      Size = 1
    end
    object Q_OCDT_CADASTRO: TDateTimeField
      FieldName = 'DT_CADASTRO'
      Origin = 'CURA.ORDEM_COMPRA.DT_CADASTRO'
      Visible = False
    end
    object Q_OCUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'CURA.ORDEM_COMPRA.USUARIO'
      Visible = False
      FixedChar = True
      Size = 50
    end
    object Q_OCCH_AUT: TIntegerField
      FieldName = 'CH_AUT'
      Origin = 'CURA.ORDEM_COMPRA.CH_AUT'
      Visible = False
    end
    object Q_OCCH_CANCEL: TIntegerField
      FieldName = 'CH_CANCEL'
      Origin = 'CURA.ORDEM_COMPRA.CH_CANCEL'
      Visible = False
    end
    object Q_OCFOR_CODI: TStringField
      FieldName = 'FOR_CODI'
      Origin = 'CURA.ORDEM_COMPRA.FOR_CODI'
      Visible = False
      FixedChar = True
      Size = 15
    end
    object Q_OCNUM_OCOM: TIntegerField
      FieldName = 'NUM_OCOM'
      Origin = 'CURA.ORDEM_COMPRA.NUM_OCOM'
      Visible = False
    end
    object Q_OCCOND_PGTO_ID: TIntegerField
      FieldName = 'COND_PGTO_ID'
      Origin = 'CURA.ORDEM_COMPRA.COND_PGTO_ID'
      Visible = False
    end
    object Q_OCOBS: TMemoField
      FieldName = 'OBS'
      Origin = 'CURA.ORDEM_COMPRA.OBS'
      Visible = False
      BlobType = ftMemo
    end
    object Q_OCDT_EMISSAO: TDateTimeField
      FieldName = 'DT_EMISSAO'
      Origin = 'CURA.ORDEM_COMPRA.DT_EMISSAO'
      Visible = False
    end
    object Q_OCVALOR: TFloatField
      FieldName = 'VALOR'
      Origin = 'CURA.ORDEM_COMPRA.VALOR'
      Visible = False
    end
    object Q_OCDT_CANCEL: TDateTimeField
      FieldName = 'DT_CANCEL'
      Origin = 'CURA.ORDEM_COMPRA.DT_CANCEL'
      Visible = False
    end
    object Q_OCVL_IPI: TFloatField
      FieldName = 'VL_IPI'
      Origin = 'CURA.ORDEM_COMPRA.VL_IPI'
      Visible = False
    end
    object Q_OCVL_TOT: TFloatField
      FieldName = 'VL_TOT'
      Origin = 'CURA.ORDEM_COMPRA.VL_TOT'
      Visible = False
    end
    object Q_OCMAT_ID: TAutoIncField
      FieldName = 'MAT_ID'
      Origin = 'CURA.MATERIAIS.MAT_ID'
      Visible = False
    end
    object Q_OCUNI_COMP_ID: TIntegerField
      DisplayWidth = 10
      FieldName = 'UNI_COMP_ID'
      Visible = False
    end
    object Q_OCIORD_ID: TAutoIncField
      FieldName = 'IORD_ID'
      Visible = False
    end
    object Q_OCTIPOCOMPRA: TStringField
      FieldName = 'TIPOCOMPRA'
      Visible = False
      FixedChar = True
      Size = 50
    end
  end
  object DS_OC: TwwDataSource
    DataSet = Q_OC
    Left = 204
    Top = 370
  end
  object Q_FORN: TwwQuery
    Active = True
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT FOR_CODI,FOR_RAZA FROM CPAFORN'
      'WHERE F_ALMOX = '#39'S'#39
      'ORDER BY FOR_RAZA')
    ValidateWithMask = True
    Left = 288
    Top = 300
    object Q_FORNFOR_RAZA: TStringField
      DisplayWidth = 50
      FieldName = 'FOR_RAZA'
      Origin = 'CURA.CPAFORN.FOR_RAZA'
      FixedChar = True
      Size = 50
    end
    object Q_FORNFOR_CODI: TStringField
      DisplayWidth = 15
      FieldName = 'FOR_CODI'
      Origin = 'CURA.CPAFORN.FOR_CODI'
      Visible = False
      FixedChar = True
      Size = 15
    end
  end
  object Q_MAT: TwwQuery
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      
        'SELECT MAT_ID, CodigoMaterial + '#39' '#39' + MAT_DESC AS MAT_DESC FROM ' +
        'MATERIAIS'
      'WHERE GENERICO NOT IN ('#39'S'#39')'
      'AND MAT_ATIVO = '#39'S'#39
      'ORDER BY MAT_DESC')
    ValidateWithMask = True
    Left = 504
    Top = 308
    object Q_MATMAT_DESC: TStringField
      DisplayWidth = 60
      FieldName = 'MAT_DESC'
      Origin = 'CURA.MATERIAIS.MAT_DESC'
      FixedChar = True
      Size = 60
    end
    object Q_MATMAT_ID: TAutoIncField
      DisplayWidth = 10
      FieldName = 'MAT_ID'
      Origin = 'CURA.MATERIAIS.MAT_ID'
      Visible = False
    end
  end
  object Q_AUX: TwwQuery
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT COUNT(*) FROM ORDEM_COMPRA'
      '')
    ValidateWithMask = True
    Left = 40
    Top = 305
    object Q_AUXCOLUMN1: TIntegerField
      FieldName = 'COLUMN1'
    end
  end
  object Q_MARC: TwwQuery
    DatabaseName = 'CURA'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT * FROM MARCAS ORDER BY MARCA_DESC       '
      '')
    ValidateWithMask = True
    Left = 639
    Top = 256
    object Q_MARCMARCA_ID: TAutoIncField
      FieldName = 'MARCA_ID'
      Origin = 'CURA.MARCAS.MARCA_ID'
    end
    object Q_MARCMARCA_DESC: TStringField
      FieldName = 'MARCA_DESC'
      Origin = 'CURA.MARCAS.MARCA_DESC'
      FixedChar = True
    end
  end
  object DS_MARC: TwwDataSource
    DataSet = Q_MARC
    Left = 636
    Top = 308
  end
  object Q_LOCAL_FILT: TwwQuery
    DatabaseName = 'Cura'
    SessionName = 'Sec_BaseFin'
    SQL.Strings = (
      'SELECT * FROM LOCAL ORDER BY DESCRICAO')
    ValidateWithMask = True
    Left = 506
    Top = 232
    object StringField7: TStringField
      DisplayWidth = 50
      FieldName = 'Descricao'
      Origin = 'CURA.LOCAL.Descricao'
      FixedChar = True
      Size = 50
    end
    object AutoIncField4: TAutoIncField
      DisplayWidth = 10
      FieldName = 'Local_ID'
      Origin = 'CURA.LOCAL.Local_ID'
      Visible = False
    end
    object StringField8: TStringField
      DisplayWidth = 1
      FieldName = 'DisponivelSN'
      Origin = 'CURA.LOCAL.DisponivelSN'
      Visible = False
      FixedChar = True
      Size = 1
    end
  end
  object UPD_OC: TUpdateSQL
    ModifySQL.Strings = (
      'update ORDEM_COMPRAITEM'
      'set'
      '  ALTERAR = :ALTERAR'
      'where'
      '  IORD_ID = :OLD_IORD_ID')
    InsertSQL.Strings = (
      'insert into ORDEM_COMPRAITEM'
      '  (ALTERAR)'
      'values'
      '  (:ALTERAR)')
    DeleteSQL.Strings = (
      'delete from ORDEM_COMPRAITEM'
      'where'
      '  IORD_ID = :OLD_IORD_ID')
    Left = 248
    Top = 388
  end
  object ADOQuery1: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT FIL_CODI, FIL_DESC FROM CPAFILI'
      'order by FIL_DESC')
    Left = 272
    Top = 56
    object ADOQuery1FIL_DESC: TStringField
      DisplayWidth = 20
      FieldName = 'FIL_DESC'
      FixedChar = True
    end
    object ADOQuery1FIL_CODI: TStringField
      DisplayWidth = 3
      FieldName = 'FIL_CODI'
      Visible = False
      FixedChar = True
      Size = 3
    end
  end
end
