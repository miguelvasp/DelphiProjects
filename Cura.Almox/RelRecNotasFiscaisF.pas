...
type
    TFLstNotasFiscaisf = class(TForm)
    ...
    Q_NotasFiscaisFOR_CODI: TStringField;
    LST_RecNotasFiscaisf: TQuickRep;
    QRBand1: TQRBand;
    QRShape1: TQRShape;
    QRLabel1: TQRLabel;
    QRLabel2: TQRLabel;
    ...
    procedure LST_RecNotasFiscaisfBeforePrint(Sender: TCustomQuickRep;
...
var
  FLstNotasFiscaisf: TFLstNotasFiscaisf;
...
procedure TFLstNotasFiscaisf.FormCreate(Sender: TObject);
var StrTipo : string ;
    StrAnd  : string ;
begin
  if RecNotasFiscais.Ordenacao = '1'  then begin
     ...
     QrLabel11.caption :='Fornecedores :' ;
     QRDBText6.DataField := 'FOR_CODI' ;
     QRGroup1.expression := 'Q_NOTASFISCAIS.FOR_CODI' ;
  end ;
...
//  QRLabel2.caption := 'Relatório de Contas a Receber por C.R. - ' + StrTipo ;
  ...
  Q_NOTASFISCAIS.SQL.ADD('B.NFI_CDPG, B.NFI_CFAT, B.FOR_CODI, ');
  Q_NOTASFISCAIS.SQL.ADD('D.CDP_DESC, E.VEN_NOME,F.CFA_DCFO ');
  Q_NOTASFISCAIS.SQL.ADD('From SIANFIS B, CPACDPG D, SIAVEND E, SIACFAT F ');
  Q_NOTASFISCAIS.SQL.ADD(' where B.FOR_CODI <> null ');
  ...
  if (FornecI <> '')     and
     (FornecF <> '')     then begin
     Q_NotasFiscais.SQL.ADD( StrANd + ' for_codi >= '+ '''' + FornecI + '''' );
     Q_NotasFiscais.SQL.ADD( ' and for_Codi <= '+''''+ FornecF +'''');
     StrAnd := ' and ' ;
  end;
...
//     end ;
  ...
  if RecNotasFiscais.Ordenacao = '1' then
     Q_NOTASFISCAIS.SQL.ADD( ' Order by for_codi' )  ;
...
procedure TFLstNotasFiscaisf.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Q_NOTASFISCAIS.close;
  action:=cafree;
end;

procedure TFLstNotasFiscaisf.LST_RecNotasFiscaisfBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
Var
     vLogo : TBlobField;
begin
