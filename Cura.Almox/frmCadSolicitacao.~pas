...
type
  TfrmCadSolicitaca = class(TForm)
    ...
    procedure qrItemBeforeDelete(DataSet: TDataSet);
  private
...
uses UMenu, UfrmRelSolicitacao, USolicitacao, UfrmBuscaMaterial;
...
procedure TfrmCadSolicitaca.btnGetMaterialClick(Sender: TObject);
begin
    frmBuscaMaterial := tfrmBuscaMaterial.create(Self);
    frmBuscaMaterial.showmodal;
    if frmBuscaMaterial.lbSelecao.Caption <> '' then begin
      if qrItemITEM.AsString = '' then
         qrItem.Append
      else
         qrItem.Edit;

      qrItemMAT_ID.Value := StrToInt(frmBuscaMaterial.lbSelecao.Caption);
    end;
    frmBuscaMaterial.lbSelecao.Caption := '';
    FreeAndNil(frmBuscaMaterial);
end;

procedure TfrmCadSolicitaca.qrItemBeforeDelete(DataSet: TDataSet);
begin
    if RzPageControl1.ActivePageIndex = 1 then begin
        with qrAux do begin
            sql.Clear;
            SQL.Add('DELETE FROM ITEM_SOLICITACAO WHERE ITEM = ' + qrItemITEM.AsString);
            ExecSQL;
        end;
