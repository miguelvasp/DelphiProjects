...
type
  TfrmSolicitacaoMateriais = class(TForm)
    ...
    qrSolicitacaoMAT_CONTROLADO: TStringField;
    ...
    procedure qrSolicitacaoAfterOpen(DataSet: TDataSet);
...
uses UMenu, UfrmRelRequisicaoUsuarios, ufrmRelSaldosRequisicao,
  UfrmAddItemSol, ufrmAdicionaItensSol, UfrmBuscaMaterial;
...
procedure TfrmSolicitacaoMateriais.qrSolicitacaoAfterOpen(
  DataSet: TDataSet);
begin
   qrMaterial.Close;
   qrMaterial.sql.Clear;
   qrMaterial.sql.Add('select MAT_ID, MAT_DESC, U.UNI_SIGLA');
   qrMaterial.sql.Add('from MATERIAIS M inner join UNIDADES U');
   qrMaterial.sql.Add('ON M.UNI_ID = U.UNI_ID');
   qrMaterial.sql.Add('where case when MAT_ATIVO IS null then ''N'' else MAT_ATIVO end = ''S''');
   qrMaterial.sql.Add('AND CASE WHEN GENERICO IS NULL THEN ''N'' ELSE GENERICO END = ''N''');
   IF qrSolicitacaoMAT_CONTROLADO.AsString = 'S' then
        qrMaterial.sql.Add('AND ISNULL(CONTROLA_GRUPO, '''' ) = ' + '''' + qrSolicitacaoMAT_CONTROLADO.AsString + '''')
   else
        qrMaterial.sql.Add('AND ISNULL(CONTROLA_GRUPO, ''N'' ) = ''N'' ') ;
   qrMaterial.sql.Add('order by mat_Desc');
   qrMaterial.Open;
end;

procedure TfrmSolicitacaoMateriais.qrSolicitacaoAfterPost(
  DataSet: TDataSet);
begin
   qrMaterial.Close;
   qrMaterial.sql.Clear;
...
procedure TfrmSolicitacaoMateriais.btnGetMaterialClick(Sender: TObject);
begin
...
procedure TfrmSolicitacaoMateriais.BitBtn1Click(Sender: TObject);
begin
    frmBuscaMaterial := tfrmBuscaMaterial.create(Self);
    //frmBuscaMaterial.chkConsideraMarca.Checked := True;
    frmBuscaMaterial.showmodal;
    if frmBuscaMaterial.lbSelecao.Caption <> '' then begin
      if qrItensITEM.AsString = '' then
         qrItens.Append
      else
         qrItens.Edit;

      qrItensMAT_ID.Value := StrToInt(frmBuscaMaterial.lbSelecao.Caption);
