   ...
   if qrImportados.Active then begin
       ...
       while not qrImportados.Eof do begin
           ...
           if qrImportadosFOR_CODI.AsString = '' then begin
               Memo1.Lines.Add('Material: ' + qrImportadoscod_bionexo.AsString + ' - Fornecedor com CNPJ: ' + qrImportadoscnpj.AsString + ' não encontrado');
                Erros := True;
           end;

           //valida condição de pagamento
           if qrImportadosPGTO.AsString = '' then begin
               Memo1.Lines.Add('Material: ' + qrImportadoscod_bionexo.AsString + ' - Condição de pagamento não encontrada: ' + qrImportadospgtoBio_desc.AsString + ' verifique se a condição de pagamento bionexo está vinculada');
               Erros := True;
           end;

           if qrImportadosMAT_ID.AsString = '' then begin
               Memo1.Lines.Add('Material: ' + qrImportadoscod_bionexo.AsString + ' não cadastrado');
               Erros := True;
           end;

           //verifica informações do pedido
           qrAuxPedido.close;
           ...
           if qrAuxPedido.IsEmpty then begin
               Memo1.Lines.Add('Material: ' + qrImportadoscod_bionexo.AsString + ' - Pedido não encontrado');
               Erros := True;
           end
           else begin
               if qrAuxPedidostatusItem.Value = 'Atendido' then begin
                   Memo1.Lines.Add('Material: ' + qrImportadoscod_bionexo.AsString + ' - Pedido já foi atendido');
                   Erros := True;
               end;
...
//           Memo1.Lines.Add('');
   ...
   if Erros then begin
       ...
       ShowMessage('A validação do arquivos apresentou erros');
   end
   else begin
       ShowMessage('Validação efetuada com sucesso, não foram encontrados erros.');
...
begin
  ...
  qrBionexo.Post;
      ...
      ON E : Exception do begin
          ShowMessage('Aconteceu um erro ao tentar executar a operação: ' + e.Message);
      end;
  end;
