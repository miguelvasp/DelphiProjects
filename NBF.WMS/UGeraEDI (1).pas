...
procedure TFGeraEDI.SpeedButton3Click(Sender: TObject);
begin
  close;

end;

procedure TFGeraEDI.BitBtn1Click(Sender: TObject);
const
  BRANCOA = ' ';
  BRANCON = '0';
  Brancos = '                                                                                                                                                                             ' ;
...
procedure TFGeraEDI.MaskEdit1Enter(Sender: TObject);
begin
     (Sender as TMaskedit).Color := ClAqua;
end;

procedure TFGeraEDI.MaskEdit2Enter(Sender: TObject);
begin
  (Sender as TMaskedit).Color := ClAqua;
end;

procedure TFGeraEDI.MaskEdit1Exit(Sender: TObject);
begin
  (Sender as TMaskedit).Color := ClWHITE;
end;

procedure TFGeraEDI.MaskEdit2Exit(Sender: TObject);
begin
     (Sender as TMaskedit).Color := ClWHITE;
end;

procedure TFGeraEDI.FormCreate(Sender: TObject);
begin
   Q_PARAMETRO.OPEN;
   t_acen.OPEN ;
   qrCliente.Open;
...
procedure TFGeraEDI.FormClose(Sender: TObject;
...
function TFGeraEDI.MUDALETR(LETRA: string): string;
var
  NOVALETRA         : string;
  s                 : string;
  M_DataLEN, i      : Integer;
...
procedure TFGeraEDI.GERA_CONEMB;
var
  linha : string;
  aux : string;
  a : Integer;
  ...
  try
     ...
     {******    Fim Registro 321   **********}
      ...
      {******    Registro 322   **********}
       ...
       qrConhecimentos.sql.Add(' SELECT     Ord_ID, ORD_CONH, ORD_DATA, ORD_VLR_PEDAGIO, ORD_VLR_DESPACHO, ORD_VLR_SECCAT, ORD_VLR_FRETE, ORD_PESO_TOTAL, '+
                               ' ORD_TOTALPREST, ORD_VLR_ICMS, ORD_ALIQ, ORD_VLR_OUTROS, VEIC_ID, MANI_ID, CLIN_ID, ord_redespacho, cfa_id , ORD_PAGO, CLIF_ID, CTE_CHAVE, CTE_PROTOCOLO '+
        ...
        begin
          ...
          if UpperCase(qrConhecimentosord_redespacho.Value) =  'S' then
            linha := linha + 'R'
          else
            linha := linha + 'N';
          ...
          //cHAVE DO CTE
           ...
           //  PROTOCOLO
            aux := Trim(qrConhecimentosCTE_PROTOCOLO.Value);
            iSize := 15 - Length(aux);
            if iSize > 0 then
            begin
               for i := 1 to iSize do
...
procedure TFGeraEDI.GERA_OCOREN;
  var linha : string;
  aux : string;
  a , i, iSize: Integer;
  filex : TStringList;
...
procedure TFGeraEDI.RadioGroup1Click(Sender: TObject);
begin
