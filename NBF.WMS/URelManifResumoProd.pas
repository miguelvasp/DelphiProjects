...
uses
  ...
  ExtCtrls, QuickRpt, Qrctrls, Db, DBTables, Wwquery, gtQrCtrls;
...
begin
      ...
      if achei then
       ...
       SALDOx :=  FManifesto.Q_RMPMRP_QTDE.asfloat - (CONTA_PALLET * (Pallet_Qtde  * Q_PRODQTDE_UVUE.AsFloat));

       //informa UE
       int_Aux := (int(CONTA_PALLET) * (Pallet_Qtde  * Q_PRODQTDE_UVUE.AsInteger))/ Q_PRODQTDE_UVUE.AsFloat;

       //qtde UE
       QRLabel23.Caption := FormatFloat('0', int_Aux);
      // UE := ue + int_Aux;
       //qtde UV
       QRLabel24.Caption := FormatFloat('0', (CONTA_PALLET * (Pallet_Qtde  * Q_PRODQTDE_UVUE.AsFloat)));
       ...
       if saldox > 0 then begin
           ...
           F_UE := F_UE + SALDOx / Q_PRODQTDE_UVUE.AsFloat ;
           //SALDO EM UV
