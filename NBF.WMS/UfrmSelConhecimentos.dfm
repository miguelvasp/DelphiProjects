object frmSelConhecimentos: TfrmSelConhecimentos
  Left = 214
  Top = 137
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Seleção de conhecimentos'
  ClientHeight = 455
  ClientWidth = 784
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 14
  object RzGroupBox1: TRzGroupBox
    Left = 0
    Top = 0
    Width = 784
    Height = 53
    Align = alTop
    Caption = 'Opções'
    TabOrder = 0
    object DBText1: TDBText
      Left = 264
      Top = 24
      Width = 65
      Height = 17
      DataField = 'OS_ID'
      DataSource = DataSource1
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 224
      Top = 24
      Width = 31
      Height = 18
      Caption = 'OS:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Verdana'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object RzBitBtn1: TRzBitBtn
      Left = 6
      Top = 17
      Width = 37
      Height = 30
      Hint = 'Marcar Todos'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = RzBitBtn1Click
      Glyph.Data = {
        
        4646464646464646464646464646464646464646464646464646}
    end
    object RzBitBtn3: TRzBitBtn
      Left = 46
      Top = 17
      Width = 37
      Height = 30
      Hint = 'Desmarcar Todos'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = RzBitBtn3Click
      Glyph.Data = {
        
        0101010101010101010101010101010101010101010101010101}
    end
    object RzBitBtn2: TRzBitBtn
      Left = 88
      Top = 17
      Width = 113
      Height = 30
      Hint = 'Desmarcar Todos'
      Caption = 'Concluir'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = RzBitBtn2Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object wwDBGrid1: TwwDBGrid
    Left = 0
    Top = 53
    Width = 784
    Height = 402
    ControlType.Strings = (
      'SEL;CheckBox;*;')
    Selected.Strings = (
      'SEL'#9'1'#9'XX'#9'F'
      'ORD_CONH'#9'9'#9'Número'#9'F'
      'CLIN_RAZA'#9'25'#9'Cliente NBF'#9'F'
      'CLIF_RAZA'#9'25'#9'Cliente Final'#9'F'
      'ORD_VLR_FRETE'#9'10'#9'Frete'#9'F'
      'ORD_VLR_OUTROS'#9'10'#9'Outros'#9'F'
      'ORD_VLR_ICMS'#9'10'#9'ICMS'#9'F'
      'ORD_TOTALPREST'#9'10'#9'Total'#9'T')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    Align = alClient
    DataSource = DataSource1
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Verdana'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    TitleAlignment = taLeftJustify
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    TitleLines = 1
    TitleButtons = False
    IndicatorColor = icBlack
  end
  object qrOCE: TADOQuery
    Connection = FMenu.ADOConnection
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'OS_ID'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT '
      'A.SEL, ORD_ID, A.OS_ID,'
      'A.ORD_DATA,'
      'A.ORD_CONH,'
      'A.ORD_NOTAS,'
      'B.CLIN_RAZA,'
      'C.CLIF_RAZA , '
      'A.ORD_VLR_FRETE, '
      'A.ORD_VLR_OUTROS, '
      'A.ORD_VLR_ICMS, '
      'A.ORD_TOTALPREST'
      
        'FROM ORDEM_COLETA_ENTREGA A  ,CLIENTENBF B,  CLIENTEFINAL C , OS' +
        ' D'
      'WHERE A.CLIN_ID = B.CLIN_ID'
      'AND  A.CLIF_ID = C.CLIF_ID'
      'AND A.OS_ID = D.OS_ID'
      'AND A.OS_ID = :OS_ID'
      'AND A.ORD_STATUS = '#39'E'#39
      'AND A.FAT_ID IS NULL'
      'ORDER BY A.ORD_CONH')
    Left = 160
    Top = 208
    object qrOCESEL: TStringField
      DisplayLabel = 'XX'
      DisplayWidth = 1
      FieldName = 'SEL'
      ProviderFlags = [pfInUpdate]
      Size = 1
    end
    object qrOCEORD_CONH: TStringField
      DisplayLabel = 'Número'
      DisplayWidth = 9
      FieldName = 'ORD_CONH'
      ProviderFlags = []
      Size = 10
    end
    object qrOCECLIN_RAZA: TStringField
      DisplayLabel = 'Cliente NBF'
      DisplayWidth = 25
      FieldName = 'CLIN_RAZA'
      ProviderFlags = []
      Size = 50
    end
    object qrOCECLIF_RAZA: TStringField
      DisplayLabel = 'Cliente Final'
      DisplayWidth = 25
      FieldName = 'CLIF_RAZA'
      ProviderFlags = []
      Size = 50
    end
    object qrOCEORD_VLR_FRETE: TFloatField
      DisplayLabel = 'Frete'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_FRETE'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object qrOCEORD_VLR_OUTROS: TFloatField
      DisplayLabel = 'Outros'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_OUTROS'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object qrOCEORD_VLR_ICMS: TFloatField
      DisplayLabel = 'ICMS'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_ICMS'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object qrOCEORD_TOTALPREST: TFloatField
      DisplayLabel = 'Total'
      DisplayWidth = 10
      FieldName = 'ORD_TOTALPREST'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object qrOCEORD_NOTAS: TStringField
      DisplayLabel = 'Notas'
      DisplayWidth = 35
      FieldName = 'ORD_NOTAS'
      ProviderFlags = []
      Visible = False
      Size = 50
    end
    object qrOCEORD_DATA: TDateTimeField
      DisplayLabel = 'Data'
      DisplayWidth = 10
      FieldName = 'ORD_DATA'
      ProviderFlags = []
      Visible = False
      DisplayFormat = 'dd/MM/yy'
    end
    object qrOCEORD_ID: TAutoIncField
      FieldName = 'ORD_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      ReadOnly = True
      Visible = False
    end
    object qrOCEOS_ID: TIntegerField
      FieldName = 'OS_ID'
    end
  end
  object DataSource1: TDataSource
    DataSet = cdsOCE
    Left = 160
    Top = 288
  end
  object dspOCE: TDataSetProvider
    DataSet = qrOCE
    Constraints = True
    UpdateMode = upWhereKeyOnly
    Left = 224
    Top = 208
  end
  object cdsOCE: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspOCE'
    AfterPost = cdsOCEAfterPost
    Left = 296
    Top = 208
    object cdsOCESEL: TStringField
      DisplayLabel = 'XX'
      DisplayWidth = 1
      FieldName = 'SEL'
      ProviderFlags = [pfInUpdate]
      OnChange = cdsOCESELChange
      Size = 1
    end
    object cdsOCEORD_CONH: TStringField
      DisplayLabel = 'Número'
      DisplayWidth = 9
      FieldName = 'ORD_CONH'
      ProviderFlags = []
      Size = 10
    end
    object cdsOCECLIN_RAZA: TStringField
      DisplayLabel = 'Cliente NBF'
      DisplayWidth = 25
      FieldName = 'CLIN_RAZA'
      ProviderFlags = []
      Size = 50
    end
    object cdsOCECLIF_RAZA: TStringField
      DisplayLabel = 'Cliente Final'
      DisplayWidth = 25
      FieldName = 'CLIF_RAZA'
      ProviderFlags = []
      Size = 50
    end
    object cdsOCEORD_VLR_FRETE: TFloatField
      DisplayLabel = 'Frete'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_FRETE'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object cdsOCEORD_VLR_OUTROS: TFloatField
      DisplayLabel = 'Outros'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_OUTROS'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object cdsOCEORD_VLR_ICMS: TFloatField
      DisplayLabel = 'ICMS'
      DisplayWidth = 10
      FieldName = 'ORD_VLR_ICMS'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object cdsOCEORD_TOTALPREST: TFloatField
      DisplayLabel = 'Total'
      DisplayWidth = 10
      FieldName = 'ORD_TOTALPREST'
      ProviderFlags = []
      DisplayFormat = '###,###,##0.00'
    end
    object cdsOCEORD_NOTAS: TStringField
      FieldName = 'ORD_NOTAS'
      ProviderFlags = []
      Visible = False
      Size = 50
    end
    object cdsOCEORD_DATA: TDateTimeField
      FieldName = 'ORD_DATA'
      ProviderFlags = []
      Visible = False
    end
    object cdsOCEORD_ID: TAutoIncField
      FieldName = 'ORD_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      ReadOnly = True
      Visible = False
    end
    object cdsOCEOS_ID: TIntegerField
      FieldName = 'OS_ID'
    end
  end
  object qrAux: TADOQuery
    Connection = FMenu.ADOConnection
    Parameters = <
      item
        Name = 'SEL'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 1
        Value = Null
      end
      item
        Name = 'os_id'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'update ordem_coleta_entrega set SEL = :SEL'
      'where os_id = :os_id'
      'and ord_status = '#39'E'#39)
    Left = 392
    Top = 208
  end
end
