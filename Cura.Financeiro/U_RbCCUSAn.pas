...
procedure TFlstRbCCUSAn.FormCreate(Sender: TObject);
var StrTipo : string ;
begin
  if Tipo = 0 then StrTipo := 'Recebidos';
  if Tipo = 1 then StrTipo := 'A Receber ';
  if Tipo = 2 then StrTipo := 'Todos';
  QRLabel2.caption := 'Relatório Analítico de Contas a Receber - CCusto ' + StrTipo ;
  Q_GerAn.close ;
  Q_GerAn.SQL.clear ;
  if (RECEBIMENTOI = 0.0) and (RECEBIMENTOF = 0.0) and (BancoContaI = '') and (BancoContaF = '') and (LOTEI = '') and (LOTEF = '' ) and (TIPO <> 0 ) then begin
     ...
     Q_GerAn.SQL.ADD( ' order by D.Fil_DESC ' )  ;
   end else begin
     Q_GerAn.SQL.ADD('Select A.*,C.CON_DESC,') ;
     Q_GerAn.SQL.ADD('C.CON_CODI,D.FIL_CODI,D.FIL_DESC, E.LCO_BANC ');
     Q_GerAn.SQL.ADD(' from CPARECE A,CPACONT C,CPAFILI D, SIALCOB E, cpaclie f  ');
...
procedure TFlstRbCCUSAn.FormClose(Sender: TObject;
...
procedure TFlstRbCCUSAn.LST_CCUSBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
Var
     vLogo : TBlobField;
begin
...
procedure TFlstRbCCUSAn.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   //fecha a query
   Q_IREC.close;
...
procedure TFlstRbCCUSAn.QRDBText15Print(sender: TObject;
  var Value: String);
begin
  fTotCger   := FtotCger + Q_IRECIRE_VTOT.asfloat ;
end;

procedure TFlstRbCCUSAn.QRExpr5Print(sender: TObject; var Value: String);
begin
...
procedure TFlstRbCCUSAn.QRExpr6Print(sender: TObject; var Value: String);
begin
