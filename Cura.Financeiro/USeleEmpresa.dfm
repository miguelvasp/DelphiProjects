object frmSeleEmpresa: TfrmSeleEmpresa
  Left = 249
  Top = 110
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Seleção de Empresas'
  ClientHeight = 534
  ClientWidth = 519
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 16
  object wwDBGrid1: TwwDBGrid
    Left = 16
    Top = 40
    Width = 489
    Height = 481
    Selected.Strings = (
      'SEL'#9'1'#9'**'#9'F'
      'EMPRESA'#9'30'#9'EMPRESA'#9'F')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    DataSource = dsEmpresa
    TabOrder = 0
    TitleAlignment = taLeftJustify
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    TitleLines = 1
    TitleButtons = False
    IndicatorColor = icBlack
  end
  object RzBitBtn1: TRzBitBtn
    Left = 360
    Top = 8
    Width = 145
    Height = 28
    Caption = 'Concluir'
    TabOrder = 1
    OnClick = RzBitBtn1Click
    Glyph.Data = {
      
      3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C}
  end
  object RzBitBtn2: TRzBitBtn
    Left = 16
    Top = 8
    Width = 161
    Height = 29
    Hint = 'Selecionar todas as empresas'
    Caption = 'Selecionar Todas'
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    OnClick = RzBitBtn2Click
    Glyph.Data = {
      
      4646464646464646464646464646464646464646464646464646}
  end
  object RzBitBtn3: TRzBitBtn
    Left = 184
    Top = 8
    Width = 169
    Height = 29
    Hint = 'Limpar Seleção'
    Caption = 'Limpar Seleção'
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = RzBitBtn3Click
    Glyph.Data = {
      
      0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D0D}
  end
  object Q_EMP: TwwQuery
    Active = True
    CachedUpdates = True
    AfterPost = Q_EMPAfterPost
    DatabaseName = 'Financeiro'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT FIL_CODI, FIL_DESC, SEL, RAZAO FROM CPAFILI'
      'ORDER BY FIL_DESC')
    ControlType.Strings = (
      'SEL;CheckBox;*;')
    ValidateWithMask = True
    Left = 192
    Top = 192
    object Q_EMPSEL: TStringField
      DisplayLabel = '**'
      DisplayWidth = 1
      FieldName = 'SEL'
      Origin = 'FINANCEIRO.CPAFILI.SEL'
      FixedChar = True
      Size = 1
    end
    object Q_EMPFIL_DESC: TStringField
      DisplayLabel = 'EMPRESA'
      DisplayWidth = 40
      FieldName = 'FIL_DESC'
      Origin = 'FINANCEIRO.CPAFILI.FIL_DESC'
      FixedChar = True
    end
    object Q_EMPRAZAO: TStringField
      DisplayLabel = 'RAZÃO SOCIAL'
      DisplayWidth = 100
      FieldName = 'RAZAO'
      Origin = 'FINANCEIRO.CPAFILI.RAZAO'
      FixedChar = True
      Size = 100
    end
    object Q_EMPFIL_CODI: TStringField
      DisplayWidth = 3
      FieldName = 'FIL_CODI'
      Origin = 'FINANCEIRO.CPAFILI.FIL_CODI'
      Visible = False
      FixedChar = True
      Size = 3
    end
  end
  object dsEmpresa: TDataSource
    DataSet = qrEmpTemp
    Left = 344
    Top = 152
  end
  object UpdateSQL1: TUpdateSQL
    ModifySQL.Strings = (
      'update EMP_TEMP'
      'set'
      '  USUARIO = :USUARIO,'
      '  FIL_CODI = :FIL_CODI,'
      '  SEL = :SEL'
      'where'
      '  id = :OLD_id')
    InsertSQL.Strings = (
      'insert into EMP_TEMP'
      '  (USUARIO, FIL_CODI, SEL)'
      'values'
      '  (:USUARIO, :FIL_CODI, :SEL)')
    DeleteSQL.Strings = (
      'delete from EMP_TEMP'
      'where'
      '  id = :OLD_id')
    Left = 192
    Top = 272
  end
  object qrEmpTemp: TwwQuery
    CachedUpdates = True
    AfterPost = qrEmpTempAfterPost
    DatabaseName = 'Financeiro'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM EMP_TEMP '
      ''
      'WHERE USUARIO = :USUARIO')
    UpdateObject = UpdateSQL1
    ControlType.Strings = (
      'SEL;CheckBox;*;')
    ValidateWithMask = True
    Left = 336
    Top = 312
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'USUARIO'
        ParamType = ptUnknown
      end>
    object qrEmpTempSEL: TStringField
      DisplayLabel = '**'
      DisplayWidth = 1
      FieldName = 'SEL'
      Origin = 'FINANCEIRO.EMP_TEMP.SEL'
      FixedChar = True
      Size = 1
    end
    object qrEmpTempEMPRESA: TStringField
      DisplayWidth = 30
      FieldKind = fkLookup
      FieldName = 'EMPRESA'
      LookupDataSet = Q_EMP
      LookupKeyFields = 'FIL_CODI'
      LookupResultField = 'FIL_DESC'
      KeyFields = 'FIL_CODI'
      Size = 30
      Lookup = True
    end
    object qrEmpTempFIL_CODI: TStringField
      DisplayWidth = 3
      FieldName = 'FIL_CODI'
      Origin = 'FINANCEIRO.EMP_TEMP.FIL_CODI'
      Visible = False
      FixedChar = True
      Size = 3
    end
    object qrEmpTempUSUARIO: TIntegerField
      FieldName = 'USUARIO'
      Origin = 'FINANCEIRO.EMP_TEMP.USUARIO'
      Visible = False
    end
    object qrEmpTempid: TAutoIncField
      FieldName = 'id'
      Origin = 'FINANCEIRO.EMP_TEMP.id'
    end
  end
  object wwQuery1: TwwQuery
    CachedUpdates = True
    AfterPost = Q_EMPAfterPost
    DatabaseName = 'Financeiro'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'DELETE FROM EMP_TEMP'
      'WHERE USUARIO = :USUARIO')
    ControlType.Strings = (
      'SEL;CheckBox;*;')
    ValidateWithMask = True
    Left = 120
    Top = 176
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'USUARIO'
        ParamType = ptUnknown
      end>
  end
  object QRADD: TwwQuery
    CachedUpdates = True
    AfterPost = Q_EMPAfterPost
    DatabaseName = 'Financeiro'
    SessionName = 'Sec_BaseFin'
    RequestLive = True
    SQL.Strings = (
      'INSERT INTO EMP_TEMP'
      'VALUES(:USUÁRIO, :FILCODI, :SEL)')
    ControlType.Strings = (
      'SEL;CheckBox;*;')
    ValidateWithMask = True
    Left = 256
    Top = 96
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'USUÁRIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FILCODI'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'SEL'
        ParamType = ptUnknown
      end>
  end
end
