...
type
  TFlstVencimentosAn = class(TForm)
    Q_CGER: TwwQuery;
...
var
  FlstVencimentosAn: TFlstVencimentosAn;
implementation
...
procedure TFlstVencimentosAn.FormCreate(Sender: TObject);
var StrTipo : String ;
begin
  if Tipo = 0 then StrTipo := 'Pagos';
  if Tipo = 1 then StrTipo := 'A Pagar ';
  if Tipo = 2 then StrTipo := 'Todos';
  QRLabel2.caption := 'Relatório Analítico de Contas a Pagar - Vencimento ' + StrTipo ;
  ...
  Q_CGER.SQL.ADD( ' order by A.Vencimento '  )  ;
  Q_CGER.open;
  fGeralCger := 0 ;
  ftotCger := 0 ;
...
procedure TFlstVencimentosAn.FormClose(Sender: TObject;
...
procedure TFlstVencimentosAn.LST_VencBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
Var
     vLogo : TBlobField;
begin
...
procedure TFlstVencimentosAn.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   //fecha a query
   Q_IPAG.close;
...
procedure TFlstVencimentosAn.QRDBText15Print(sender: TObject;
  var Value: String);
begin
  fTotCger   := FtotCger + Q_IPAGValorPago.asfloat ;
end;

procedure TFlstVencimentosAn.QRExpr2Print(sender: TObject;
  var Value: String);
begin
...
procedure TFlstVencimentosAn.QRExpr4Print(sender: TObject;
  var Value: String);
begin
