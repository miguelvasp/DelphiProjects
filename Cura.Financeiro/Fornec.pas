...
begin
   ...
   //Faz com que os outros bancos não sejam padrão, caso este seja.
   //Se nenhum banco for padrão, quando for buscar no Sispag pego o primeiro banco
...
begin
   If Q_FBABAN_ID.AsString = '' then Begin
      MessageDlg('O código do banco é de preenchimento obrigatório.',mtWarning,[mbok],0);
      Abort;
   End;
   If Q_FBABAN_AGEN.AsString = '' then Begin
      MessageDlg('O Número da Agencia é de preenchimento obrigatório.',mtWarning,[mbok],0);
      Abort;
   End;
   If Q_FBABAN_CONTA.AsString = '' then Begin
      MessageDlg('O Número da Conta Corrente é de preenchimento obrigatório.',mtWarning,[mbok],0);
      Abort;
   End;

   if Q_FBABAN_DIGITOCC.AsString = '' then
   begin
      MessageDlg('O Digito da Conta Corrente é de preenchimento obrigatório.',mtWarning,[mbok],0);
      Abort;
   end;
