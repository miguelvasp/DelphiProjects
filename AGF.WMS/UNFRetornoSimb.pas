...
//  IF qrClienteUF_SIGLA.Value = 'SP' then
  ...
  while not qrItensGeracao.Eof do begin
        ...
        mes := '';
        mes := 'Erro: Produto: '+qraux.fieldByName('PRO_COD').AsString + ', quantidade em notas de saída: ' + qrItensGeracaoQTDE.AsString;
...
begin
   ...
   Q_aux.ExecSql;
    ...
    IF Q_NF2.recordcount = 0 then begin
       MessageDlg('Não existe Nota Fiscal de Saída para os filtros escolhidos!  ',mtWarning,[mbok],0);
       wwDBLookupCombo4.setfocus ;
       abort ;
    end ;
    ...
    Q_aux.ExecSql;
        try
          ...
          End
           ...
           begin
             MessageDlg('Checagem de Notas Fiscais de Saída, com erro !',mtInformation,[mbOk], 0);
             abort  ;
           end;
        end;
        ...
        Q_NFRS2.open ;
         ...
         IF (Q_AUX.fieldbyname('qtde').asinteger = 0)  then begin
             MessageDlg('Todas as Notas Fiscais de Saída foram rejeitadas! ',mtWarning,[mbok],0);
             abort ;
         end ;
...
begin
        if MessageDlg('Confirma Geração da Nota Fiscal de Retorno Simbólico ?'
        ...
        Begin
          ...
          End
           ...
           begin
             MessageDlg('Geração da Nota Fiscal de Retorno Simbólico, com erro !',mtInformation,[mbOk], 0);
             abort  ;
           end;
         end;
           ...
           While not Q_aux.eof do begin
                    ...
                    begin
                       MessageDlg('Geração do Cálculo da Nota Fiscal, com erro !',mtInformation,[mbOk], 0);
                       abort  ;
                    end;
                  end;
            ...
            MessageDlg(' Nota Fiscal de Retorno Simbólico foi gerada com sucesso !',mtInformation,[mbOk], 0);
...
begin
     if MessageDlg('Confirma Geração das de Retorno Simbólico para as notas não rejeitadas?' ,mtConfirmation, [mbYes, mbNo], 0) = mrYES then  begin
        //LIMPA A SELEÇÃO
